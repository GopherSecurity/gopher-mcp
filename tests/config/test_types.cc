...
TEST_F(ConfigTypesTest, NodeConfigValidation_MetadataKeyTooLong) {
    ...
    try {
        ...
        EXPECT_NE(std::string(e.what()).find("exceeds maximum length"), std::string::npos);
    }
}
...
TEST_F(ConfigTypesTest, AdminConfigValidation_InvalidPathPrefix) {
    ...
    try {
        ...
        EXPECT_NE(std::string(e.what()).find("must start with '/'"), std::string::npos);
    }
}
...
TEST_F(ConfigTypesTest, AdminConfigValidation_CorsWithoutOrigins) {
    ...
    try {
        ...
        EXPECT_NE(std::string(e.what()).find("must be specified"), std::string::npos);
    }
}
...
TEST_F(ConfigTypesTest, AdminConfigJsonRoundTrip) {
    ...
    original.cors_origins = {"http://localhost:3000", "https://admin.example.com"};
...
TEST_F(ConfigTypesTest, BootstrapConfigValidation_InvalidVersion) {
    ...
    try {
        ...
        EXPECT_EQ(e.field(), "version");
        EXPECT_NE(std::string(e.what()).find("Invalid version format"), std::string::npos);
    }
...
TEST_F(ConfigTypesTest, BootstrapConfigMerge) {
    ...
    EXPECT_EQ(base.admin.allowed_ips, (std::vector<std::string>{"192.168.0.0/16"}));
}
...
TEST_F(ConfigTypesTest, JsonParsingWithNullValues) {
    ...
    json with_nulls = {
        ...
        {"node", {
            {"id", "test-node"},
            {"cluster", nullptr},  // Null value should use default
            {"region", nullptr}
        }},
        {"admin", {
            {"address", nullptr},
