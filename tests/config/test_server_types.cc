...
TEST_F(FilterConfigTest, DefaultValues) {
    ...
    EXPECT_TRUE(fc.config.is_object());
...
TEST_F(FilterConfigTest, JsonSerialization) {
    ...
    auto json = fc.toJson();
    EXPECT_EQ(json["type"], "rate_limit");
    EXPECT_EQ(json["name"], "api_rate_limit");
    EXPECT_EQ(json["config"]["requests_per_second"], 100);
    EXPECT_EQ(json["config"]["burst_size"], 200);
...
TEST_F(FilterChainConfigTest, JsonSerialization) {
    ...
    EXPECT_EQ(json["name"], "http_chain");
    EXPECT_EQ(json["transport_type"], "http");
    EXPECT_EQ(json["filters"].size(), 2);
