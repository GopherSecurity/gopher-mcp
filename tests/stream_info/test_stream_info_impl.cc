...
protected:
  void SetUp() override {
    stream_info_ = StreamInfoImpl::create();
  }
...
TEST_F(FilterStateImplTest, MultipleObjects) {
  ...
  // Verify values
  auto* retrieved1 = dynamic_cast<const TestFilterStateObject*>(filter_state_.getData("obj1"));
  auto* retrieved2 = dynamic_cast<const TestFilterStateObject*>(filter_state_.getData("obj2"));
  auto* retrieved3 = dynamic_cast<const TestFilterStateObject*>(filter_state_.getData("obj3"));
...
TEST_F(FilterStateImplTest, OverwriteData) {
  ...
  auto* retrieved1 = dynamic_cast<const TestFilterStateObject*>(filter_state_.getData("key"));
  ...
  // Verify new value
  auto* retrieved2 = dynamic_cast<const TestFilterStateObject*>(filter_state_.getData("key"));
...
TEST_F(StreamInfoImplTest, DynamicMetadataIntegration) {
  ...
  EXPECT_EQ("/api/v1/users", *stream_info_->dynamicMetadata().getMetadata("request_path"));
}
