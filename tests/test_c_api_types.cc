...
TEST_F(MCPCApiTypesTest, TextContentWithAnnotations) {
    ...
    mcp_content_block_t* block = mcp_text_content_with_annotations(
...
TEST_F(MCPCApiTypesTest, EmbeddedResourceContent) {
    ...
    mcp_resource_t resource = {
        .uri = const_cast<char*>("file:///doc.md"),
    ...
    mcp_content_block_t* block = mcp_embedded_resource_create(
        &resource, text, 1);
...
TEST_F(MCPCApiTypesTest, ToolCreation) {
    ...
    mcp_tool_t* tool = mcp_tool_complete(
        "calculator",
...
TEST_F(MCPCApiTypesTest, PromptCreation) {
    ...
    mcp_prompt_t* prompt = mcp_prompt_with_arguments(
        "text_processor", args, 2);
...
TEST_F(MCPCApiTypesTest, ErrorCreation) {
    ...
    mcp_error_t* error = mcp_error_with_data(
        -32600, "Invalid Request", data);
...
TEST_F(MCPCApiTypesTest, RequestResponseCreation) {
    ...
    // Create notification
    mcp_notification_t* notification = mcp_notification_with_params(
        "test/notification", params);
...
TEST_F(MCPCApiTypesTest, ModelPreferences) {
    ...
    mcp_model_preferences_t* prefs = mcp_model_preferences_with_priorities(
        0.3, 0.5, 0.8);
...
TEST_F(MCPCApiTypesTest, ImplementationInfo) {
    ...
    mcp_implementation_t* impl = mcp_implementation_create(
        "MCP C++ SDK", "1.0.0");
