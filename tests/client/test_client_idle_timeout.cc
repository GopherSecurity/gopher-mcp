...
class ClientIdleTimeoutTest : public ::testing::Test {
 protected:
  void SetUp() override {
    dispatcher_ = std::make_unique<event::LibeventDispatcher>("test-idle-timeout");
  }
...
TEST_F(ClientIdleTimeoutTest, NoSpuriousReconnectDuringRequestCycle) {
  ...
  auto idle_at_send = std::chrono::duration_cast<std::chrono::seconds>(
      t5 - activity_time_after_send).count();
...
/**
 * Test: Multiple sequential requests with gaps should not trigger stale detection
 *
 * Timeline:
...
TEST_F(ClientIdleTimeoutTest, SequentialRequestsWithGaps) {
  ...
  for (const auto& event : timeline) {
    ...
    if (event.type == "send") {
      ...
      auto idle_before_update = std::chrono::duration_cast<std::chrono::seconds>(
          event_time - last_activity).count();
      ...
      EXPECT_EQ(idle_after_update, 0)
        << "At t=" << event.time_sec << "s: Idle should be 0 after activity update";
...
/**
 * Test: Long-running request (10s server processing) should not trigger reconnect
 *
 * Timeline:
...
TEST_F(ClientIdleTimeoutTest, LongRunningRequestNoReconnect) {
  ...
  for (int check_sec : check_times) {
    ...
    auto idle = std::chrono::duration_cast<std::chrono::seconds>(
        check_time - activity_at_send).count();
    ...
    if (check_sec < 30) {
      EXPECT_LT(idle, 30)
        << "At t=" << check_sec << "s: Connection should NOT be stale";
    }
  }
  ...
  auto idle_after_response = std::chrono::duration_cast<std::chrono::seconds>(
      t10 - activity_at_response).count();
...
TEST_F(ClientIdleTimeoutTest, ConnectionStaleAfter30Seconds) {
  ...
  for (const auto& check : checks) {
    ...
    auto idle = std::chrono::duration_cast<std::chrono::seconds>(
        check_time - last_activity).count();
...
TEST_F(ClientIdleTimeoutTest, BurstyRequestPattern) {
  ...
  std::vector<Request> requests = {
    ...
    {1, 2},
    {2, 3},
    {10, 4},   // 8s gap from last request at t=2
    {11, 5},   // Burst 2
  ...
  for (const auto& req : requests) {
    ...
    auto gap = std::chrono::duration_cast<std::chrono::seconds>(
        req_time - last_activity).count();

    // Verify gap is less than 30s (should not trigger stale detection)
    EXPECT_LT(gap, 30)
      << "Request " << req.request_id << " at t=" << req.time_sec
      << "s: Gap of " << gap << "s should not trigger stale detection";
    ...
    auto idle = std::chrono::duration_cast<std::chrono::seconds>(
        req_time - last_activity).count();
    EXPECT_EQ(idle, 0)
      << "Request " << req.request_id << ": Idle should be 0 after send";
  }
}
...
TEST_F(ClientIdleTimeoutTest, ExactlyThirtySecondGapIsStale) {
  ...
  EXPECT_EQ(idle, 30)
    << "Idle time should be exactly 30 seconds";
}
...
TEST_F(ClientIdleTimeoutTest, ZeroGapRequests) {
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    auto idle = std::chrono::duration_cast<std::chrono::seconds>(
        base_time - last_activity).count();

    EXPECT_EQ(idle, 0)
      << "Request " << i << ": Zero-gap requests should have 0 idle time";
  }
}
...
TEST_F(ClientIdleTimeoutTest, OldBehaviorCausedSpuriousReconnect) {
  ...
  auto idle_old = std::chrono::duration_cast<std::chrono::seconds>(
      t5 - last_activity_old_behavior).count();
  ...
  auto idle_new = std::chrono::duration_cast<std::chrono::seconds>(
      t5 - last_activity_new_behavior).count();
...
TEST_F(ClientIdleTimeoutTest, SseStreamWithInfrequentEvents) {
  ...
  for (const auto& event : timeline) {
    ...
    EXPECT_LT(gap, 30)
      << "At t=" << event.time_sec << "s (" << event.description << "): "
      << "Gap of " << gap << "s should not trigger stale detection";
...
TEST_F(ClientIdleTimeoutTest, SlowToolListingNoReconnect) {
  ...
  for (int check_sec : {1, 2, 3, 4, 5, 6, 7, 8}) {
    ...
    EXPECT_LT(idle, 30)
      << "At t=" << check_sec << "s: Should NOT be stale (idle=" << idle << "s)";
  }
...
TEST_F(ClientIdleTimeoutTest, MultipleClientActivityPatterns) {
  ...
  for (const auto& pattern : patterns) {
    ...
    for (int req_time : pattern.request_times) {
      ...
      auto gap = std::chrono::duration_cast<std::chrono::seconds>(
          req_time_point - last_activity).count();

      // All patterns should work (gaps < 30s)
      EXPECT_LT(gap, 30)
        << "Client '" << pattern.name << "' at t=" << req_time
        << "s: Gap " << gap << "s should not trigger reconnect";
