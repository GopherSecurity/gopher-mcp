 ...
 * @brief Comprehensive unit tests for mcp_filter_buffer.cc with RAII enforcement
 *
...
class BufferGuard {
 public:
  explicit BufferGuard(mcp_buffer_handle_t handle = 0) 
      : handle_(handle) {
    if (handle_ != 0) {
...
class BufferPoolGuard {
 public:
  explicit BufferPoolGuard(mcp_buffer_pool_t pool = nullptr)
      : pool_(pool) {}
...
class MCPFilterBufferTest : public ::testing::Test {
 protected:
  ...
  BufferGuard CreateBuffer(size_t capacity = 1024) {
    auto handle = mcp_buffer_create_owned(capacity, MCP_BUFFER_OWNERSHIP_EXCLUSIVE);
...
TEST_F(MCPFilterBufferTest, CreateOwnedBuffer) {
  BufferGuard buffer(mcp_buffer_create_owned(1024, MCP_BUFFER_OWNERSHIP_EXCLUSIVE));
...
TEST_F(MCPFilterBufferTest, AddBuffer) {
  ...
  char combined[100] = {};
  EXPECT_EQ(mcp_buffer_peek(buffer1, 0, combined, mcp_buffer_length(buffer1)), MCP_OK);
...
TEST_F(MCPFilterBufferTest, PrependData) {
  ...
  char result[100] = {};
  EXPECT_EQ(mcp_buffer_peek(buffer, 0, result, mcp_buffer_length(buffer)), MCP_OK);
...
TEST_F(MCPFilterBufferTest, GetContiguous) {
  ...
  EXPECT_EQ(mcp_buffer_get_contiguous(buffer, 0, 100, &ptr, &actual_length), MCP_OK);
...
TEST_F(MCPFilterBufferTest, GetContiguousWithOffset) {
  ...
  EXPECT_EQ(mcp_buffer_get_contiguous(buffer, 5, 3, &ptr, &actual_length), MCP_OK);
  ASSERT_NE(ptr, nullptr);
...
TEST_F(MCPFilterBufferTest, InvalidIntegerSize) {
  ...
  EXPECT_EQ(mcp_buffer_write_le_int(buffer, value, 3), MCP_ERROR_INVALID_ARGUMENT);
  EXPECT_EQ(mcp_buffer_write_be_int(buffer, value, 5), MCP_ERROR_INVALID_ARGUMENT);
  
  EXPECT_EQ(mcp_buffer_write_le_int(buffer, value, 4), MCP_OK);
  
  EXPECT_EQ(mcp_buffer_read_le_int(buffer, 3, &value), MCP_ERROR_INVALID_ARGUMENT);
  EXPECT_EQ(mcp_buffer_read_be_int(buffer, 7, &value), MCP_ERROR_INVALID_ARGUMENT);
}
...
TEST_F(MCPFilterBufferTest, SearchPattern) {
  ...
  // Pattern not found
  EXPECT_EQ(mcp_buffer_search(buffer, "cat", 3, 0, &position), MCP_ERROR_NOT_FOUND);
}
...
TEST_F(MCPFilterBufferTest, BufferPoolTrim) {
  ...
  size_t free_count = 0;
  EXPECT_EQ(mcp_buffer_pool_get_stats(pool, &free_count, nullptr, nullptr), MCP_OK);
...
TEST_F(MCPFilterBufferTest, NullPointers) {
  ...
  EXPECT_EQ(mcp_buffer_add(buffer, nullptr, 10), MCP_OK);  // Null data is OK with length 0
  EXPECT_EQ(mcp_buffer_add_string(buffer, nullptr), MCP_ERROR_INVALID_ARGUMENT);
  
  EXPECT_EQ(mcp_buffer_reserve(buffer, 100, nullptr), MCP_ERROR_INVALID_ARGUMENT);
...
TEST_F(MCPFilterBufferTest, LargeDataOperations) {
  ...
  EXPECT_EQ(mcp_buffer_add(buffer, large_data.data(), large_data.size()), MCP_OK);
...
TEST_F(MCPFilterBufferTest, CompleteWorkflow) {
  ...
  ASSERT_EQ(mcp_buffer_commit_reservation(&reservation, strlen(reserved_data)), MCP_OK);
  ...
  // Read back text
  char text[100] = {};
  EXPECT_EQ(mcp_buffer_peek(buffer, 0, text, strlen("Initial reserved data")), MCP_OK);
...
TEST_F(MCPFilterBufferTest, TransactionBasedOperations) {
  ...
  BufferGuard buffer1(mcp_buffer_create_owned(1024, MCP_BUFFER_OWNERSHIP_EXCLUSIVE));
  BufferGuard buffer2(mcp_buffer_create_owned(1024, MCP_BUFFER_OWNERSHIP_EXCLUSIVE));
  ...
  // For testing, we need to clean them up manually since we didn't transfer ownership
}
