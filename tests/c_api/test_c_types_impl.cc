...
TEST_F(MCPTypesTest, ToolInputSchema) {
    ...
    mcp_tool_input_schema_add_property(schema, "param1", "{\"type\": \"string\"}");
    mcp_tool_input_schema_add_property(schema, "param2", "{\"type\": \"number\"}");
...
TEST_F(MCPTypesTest, InitializeRequestCreate) {
    ...
    auto request = mcp_initialize_request_create(protocol, client_name, client_version);
    ASSERT_NE(request, nullptr);
    ...
    EXPECT_STREQ(mcp_initialize_request_get_client_version(request), client_version);
...
TEST_F(MCPTypesTest, InitializeResponseCreate) {
    ...
    auto response = mcp_initialize_response_create(protocol, server_name, server_version);
    ASSERT_NE(response, nullptr);
    
    EXPECT_STREQ(mcp_initialize_response_get_protocol_version(response), protocol);
    EXPECT_STREQ(mcp_initialize_response_get_server_name(response), server_name);
    EXPECT_STREQ(mcp_initialize_response_get_server_version(response), server_version);
