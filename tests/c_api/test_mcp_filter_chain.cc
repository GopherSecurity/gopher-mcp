...
protected:
    ...
    BufferGuard createTestBuffer(size_t size = 1024) {
        return BufferGuard(size);
    }
...
TEST_F(MCPFilterChainTest, DispatcherOperations) {
    ...
    auto timer_id = mcp_dispatcher_create_timer(dispatcher_->get(), 
                                                nullptr, nullptr);
...
TEST_F(MCPFilterChainTest, TransactionBasedOperations) {
    ...
    auto result = mcp_transaction_add(transaction, 
                                      reinterpret_cast<void*>(buffer1.get()), 
                                      MCP_TYPE_UNKNOWN);
    EXPECT_EQ(result, MCP_OK);
    
    result = mcp_transaction_add(transaction,
                                 reinterpret_cast<void*>(buffer2.get()),
                                 MCP_TYPE_UNKNOWN);
    EXPECT_EQ(result, MCP_OK);
...
TEST_F(MCPFilterChainTest, FullLifecycleWithRAII) {
    ...
    for (int i = 0; i < 5; ++i) {
        auto timer_id = mcp_dispatcher_create_timer(dispatcher_->get(),
                                                    nullptr, nullptr);
        if (timer_id != 0) {
            mcp_dispatcher_enable_timer(dispatcher_->get(), timer_id, 100 * (i + 1), false);
