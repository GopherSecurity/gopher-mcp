...
// Forward declare the init/shutdown/dispatcher functions to avoid header conflicts
extern "C" {
...
TEST_F(UnifiedChainHandleTest, AdvancedChainViaJSON) {
  ...
  mcp_filter_chain_t chain_handle = mcp_chain_create_from_json(dispatcher_, json_config);
  
  // Clean up JSON
  mcp_json_free(json_config);
  
  // If the chain creation failed (likely due to missing registry), that's ok for this test
  // We're mainly testing that the handle system doesn't crash
  if (chain_handle != 0) {
...
TEST_F(UnifiedChainHandleTest, MixedChainHandles) {
  ...
  mcp_filter_chain_t advanced_chain = mcp_chain_create_from_json(dispatcher_, json_config);
  mcp_json_free(json_config);
  ...
  if (advanced_chain != 0) {
    ...
    if (simple_dump) free(simple_dump);
    if (advanced_dump) free(advanced_dump);
