...
TEST_F(MCPCollectionsTest, MapSetGetHas) {
    ...
    EXPECT_EQ(mcp_map_set(nullptr, "key", (void*)value1), MCP_ERROR_INVALID_ARGUMENT);
    EXPECT_EQ(mcp_map_set(map, nullptr, (void*)value1), MCP_ERROR_INVALID_ARGUMENT);
...
TEST_F(MCPCollectionsTest, JSONObject) {
    ...
    // Verify values
    EXPECT_DOUBLE_EQ(mcp_json_get_number(mcp_json_object_get(object, "number")), 42.0);
    EXPECT_STREQ(mcp_json_get_string(mcp_json_object_get(object, "string")), "hello");
    ...
    EXPECT_DOUBLE_EQ(mcp_json_get_number(mcp_json_object_get(object, "number")), 100.0);
    
    // Invalid operations
    EXPECT_EQ(mcp_json_object_set(nullptr, "key", prop1), MCP_ERROR_INVALID_ARGUMENT);
    EXPECT_EQ(mcp_json_object_set(object, nullptr, prop1), MCP_ERROR_INVALID_ARGUMENT);
    EXPECT_EQ(mcp_json_object_set(object, "key", nullptr), MCP_ERROR_INVALID_ARGUMENT);
...
TEST_F(MCPCollectionsTest, JSONNested) {
    ...
    mcp_json_value_t nested_bool = mcp_json_object_get(retrieved_nested, "nested");
...
TEST_F(MCPCollectionsTest, Metadata) {
    ...
    // Verify data
    EXPECT_STREQ(mcp_json_get_string(mcp_json_object_get(retrieved, "version")), "1.0");
    EXPECT_DOUBLE_EQ(mcp_json_get_number(mcp_json_object_get(retrieved, "timestamp")), 1234567890);
    ...
    EXPECT_EQ(mcp_metadata_from_json(metadata, nullptr), MCP_ERROR_INVALID_ARGUMENT);
...
TEST_F(MCPCollectionsTest, JSONStressTest) {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        // Check data
        EXPECT_DOUBLE_EQ(mcp_json_get_number(mcp_json_object_get(nested, "index")), i);
        EXPECT_STREQ(mcp_json_get_string(mcp_json_object_get(nested, "name")), key.c_str());
    ...
    for (int i = 0; i < num_threads; ++i) {
        threads.emplace_back([&maps, i]() {
            ...
            for (int j = 0; j < ops_per_thread; ++j) {
                ...
                if (j % 50 == 0 && j > 0) {
                    std::string old_key = "thread_" + std::to_string(i) + "_key_" + std::to_string(j - 50);
        ...
        threads.emplace_back([&jsons, i]() {
            ...
            for (int j = 0; j < ops_per_thread; ++j) {
                ...
                if (j % 4 == 0) {
                    ...
                    mcp_json_object_set(root, key.c_str(), mcp_json_create_string(key.c_str()));
                } else if (j % 4 == 2) {
                    mcp_json_object_set(root, key.c_str(), mcp_json_create_bool(j % 2 == 0 ? MCP_TRUE : MCP_FALSE));
                } else {
                    mcp_json_value_t array = mcp_json_create_array();
                    for (int k = 0; k < 5; ++k) {
