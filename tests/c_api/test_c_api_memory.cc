...
TEST_F(MCPCApiMemoryTest, NestedStructureMemory) {
    ...
    auto* embedded = mcp_embedded_resource_create(
        &resource, nested.data(), nested_count);
...
TEST_F(MCPCApiMemoryTest, MemoryPoolStress) {
    ...
    for (int i = 0; i < num_iterations; ++i) {
        ...
        if (action < 2 && allocations.size() < 100) {
            ...
            mcp_memory_pool_free(pool, allocations[idx].first, allocations[idx].second);
...
TEST_F(MCPCApiMemoryTest, DeepCopyIndependence) {
    ...
    auto* original = mcp_text_content_with_annotations(
        "Original text", audience, 1, 0.5);
...
TEST_F(MCPCApiMemoryTest, SchemaMemoryLeaks) {
    ...
    for (int i = 0; i < iterations; ++i) {
        // String schema
        auto* str_schema = mcp_string_schema_with_constraints(
            "Description", "[a-z]+", 1, 100);
        mcp_string_schema_free(str_schema);
        
        // Number schema
        auto* num_schema = mcp_number_schema_with_constraints(
            "Number", 0.0, 100.0, 0.1);
        ...
        auto* enum_schema = mcp_enum_schema_with_description(
            values, 3, "Enum");
