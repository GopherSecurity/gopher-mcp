...
TEST_F(MCPJsonSimpleTest, CreateArray) {
    ...
    EXPECT_EQ(mcp_json_array_append(array, mcp_json_create_string("two")), MCP_OK);
    EXPECT_EQ(mcp_json_array_append(array, mcp_json_create_bool(MCP_TRUE)), MCP_OK);
...
TEST_F(MCPJsonSimpleTest, CreateObject) {
    ...
    // Add properties
    EXPECT_EQ(mcp_json_object_set(obj, "name", mcp_json_create_string("test")), MCP_OK);
    EXPECT_EQ(mcp_json_object_set(obj, "value", mcp_json_create_number(42)), MCP_OK);
    EXPECT_EQ(mcp_json_object_set(obj, "active", mcp_json_create_bool(MCP_TRUE)), MCP_OK);
    ...
    // Update property
    EXPECT_EQ(mcp_json_object_set(obj, "value", mcp_json_create_number(100)), MCP_OK);
...
TEST_F(MCPJsonSimpleTest, MixedNesting) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        mcp_json_object_set(item, "active", mcp_json_create_bool(i % 2 == 0 ? MCP_TRUE : MCP_FALSE));
...
TEST_F(MCPJsonSimpleTest, NullParameterHandling) {
    ...
    EXPECT_EQ(mcp_json_array_append(nullptr, mcp_json_create_null()), MCP_ERROR_INVALID_ARGUMENT);
    ...
    EXPECT_EQ(mcp_json_object_set(nullptr, "key", mcp_json_create_null()), MCP_ERROR_INVALID_ARGUMENT);
    
    mcp_json_value_t obj = mcp_json_create_object();
    EXPECT_EQ(mcp_json_object_set(obj, nullptr, mcp_json_create_null()), MCP_ERROR_INVALID_ARGUMENT);
    EXPECT_EQ(mcp_json_object_set(obj, "key", nullptr), MCP_ERROR_INVALID_ARGUMENT);
...
TEST_F(MCPJsonSimpleTest, StressLargeObject) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        EXPECT_EQ(mcp_json_object_set(obj, key.c_str(), mcp_json_create_number(i)), MCP_OK);
    }
