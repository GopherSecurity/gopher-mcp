 ...
 * @brief Comprehensive tests for TCP echo client implementation using MCP abstractions
 * 
 * Tests cover happy paths, edge cases, and thread safety with MCP network layer.
 */
...
public:
  ...
  void setConnection(network::Connection* conn) {
    connection_ = conn;
  }
...
protected:
  ...
  std::unique_ptr<network::ConnectionSocket> createClientSocket(
    ...
    auto remote_addr = network::Address::parseInternetAddress(server_addr, server_port);
...
TEST_F(TcpEchoClientBasicTest, ErrorHandlingInvalidAddresses) {
  ...
  // Test with hostname (not an IP) - this might work depending on implementation
...
TEST_F(TcpEchoClientBasicTest, StressTestRapidConnectDisconnect) {
  ...
  for (int i = 0; i < num_cycles; i++) {
    ...
    dispatcher_->post([&connect_count]() {
      connect_count++;
    });
    
    // Simulate rapid disconnect
    dispatcher_->post([&disconnect_count]() {
      disconnect_count++;
    });
  }
