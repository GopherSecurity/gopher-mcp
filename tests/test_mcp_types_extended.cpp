...
TEST_F(MCPTypesExtendedTest, ExtendedLoggingLevels) {
  ...
  // Test all levels
  EXPECT_STREQ(LoggingLevelExtended::to_string(LoggingLevelExtended::DEBUG), "debug");
  EXPECT_STREQ(LoggingLevelExtended::to_string(LoggingLevelExtended::CRITICAL), "critical");
  EXPECT_STREQ(LoggingLevelExtended::to_string(LoggingLevelExtended::EMERGENCY), "emergency");
...
TEST_F(MCPTypesExtendedTest, ClientCapabilities) {
  auto caps = build_client_capabilities()
...
TEST_F(MCPTypesExtendedTest, ProgressNotification) {
  auto notif = make_progress_notification(make_progress_token("task-123"), 0.75);
...
TEST_F(MCPTypesExtendedTest, LoggingNotification) {
  auto notif = make_log_notification(
      enums::LoggingLevelExtended::WARNING,
...
TEST_F(MCPTypesExtendedTest, MessageUnions) {
  ...
  // Server notification
  ServerNotification notif = make_progress_notification(
      make_progress_token(42), 0.5);
  
  bool is_progress = match(notif,
      [](const ProgressNotification&) { return true; },
...
TEST_F(MCPTypesExtendedTest, IntegrationRequestResponse) {
  ...
  init_result.capabilities = build_server_capabilities()
      ...
      .logging(true)
      .build();
  init_result.serverInfo = make_optional(
      make_implementation("test-server", "1.0.0"));
  ...
  tool_result.content.push_back(ExtendedContentBlock(TextContent("Found 42 users")));
