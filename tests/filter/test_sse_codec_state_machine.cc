...
protected:
  void SetUp() override {
    ...
    state_machine_ = std::make_unique<SseCodecStateMachine>(*dispatcher_, config_);
  }
...
TEST_F(SseCodecStateMachineTest, KeepAliveStopsWhenSendingEvent) {
  ...
  config_.keep_alive_callback = [&keep_alive_count]() {
    keep_alive_count++;
  };
  state_machine_ = std::make_unique<SseCodecStateMachine>(*dispatcher_, config_);
...
TEST_F(SseCodecStateMachineTest, EntryExitActions) {
  ...
  state_machine_->setEntryAction(SseCodecState::StreamActive,
    [&entry_called](SseCodecState state, std::function<void()> done) {
      entry_called = true;
      done();
    });
  
  state_machine_->setExitAction(SseCodecState::Idle,
...
TEST_F(SseCodecStateMachineTest, FormatEventWithId) {
  ...
  EXPECT_TRUE(formatted.size() >= 2 && formatted.substr(formatted.size() - 2) == "\n\n");
}
...
TEST_F(SseCodecStateMachineTest, StateNames) {
  ...
  EXPECT_EQ(SseCodecStateMachine::getStateName(SseCodecState::StreamActive), "StreamActive");
  EXPECT_EQ(SseCodecStateMachine::getStateName(SseCodecState::SendingEvent), "SendingEvent");
  EXPECT_EQ(SseCodecStateMachine::getStateName(SseCodecState::Closed), "Closed");
  EXPECT_EQ(SseCodecStateMachine::getStateName(SseCodecState::Error), "Error");
}

TEST_F(SseCodecStateMachineTest, EventNames) {
  EXPECT_EQ(SseCodecStateMachine::getEventName(SseCodecEvent::StartStream), "StartStream");
  EXPECT_EQ(SseCodecStateMachine::getEventName(SseCodecEvent::SendEvent), "SendEvent");
  EXPECT_EQ(SseCodecStateMachine::getEventName(SseCodecEvent::EventSent), "EventSent");
  EXPECT_EQ(SseCodecStateMachine::getEventName(SseCodecEvent::KeepAliveTimer), "KeepAliveTimer");
  EXPECT_EQ(SseCodecStateMachine::getEventName(SseCodecEvent::StreamError), "StreamError");
  EXPECT_EQ(SseCodecStateMachine::getEventName(SseCodecEvent::CloseStream), "CloseStream");
...
TEST_F(SseCodecStateMachineTest, ClearStateChangeListeners) {
  ...
  state_machine_->addStateChangeListener(
    [&call_count](const SseCodecStateTransitionContext&) {
      call_count++;
    });
