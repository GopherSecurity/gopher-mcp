  ...
  void testFilterChain(bool is_server) {
    executeInDispatcher([this, is_server]() {
      ...
      if (is_server) {
        ...
        test_request = "POST /mcp/v1/sse HTTP/1.1\r\n"
                      "Host: localhost\r\n"
...
TEST_F(McpHttpFilterChainFactoryTest, CreateNetworkFilterChain) {
  executeInDispatcher([this]() {
    ...
    bool result = factory.createNetworkFilterChain(
        connection->filterManager(), factories);
...
TEST_F(McpHttpFilterChainFactoryTest, CreateListenerFilterChain) {
  executeInDispatcher([this]() {
    ...
    EXPECT_FALSE(factory.createListenerFilterChain(
...
TEST_F(McpHttpFilterChainFactoryTest, FilterLifetimeManagement) {
  executeInDispatcher([this]() {
    ...
    std::string test_request = "POST /mcp/v1/sse HTTP/1.1\r\n"
                               "Host: localhost\r\n"
                               "\r\n";
    OwnedBuffer buffer;
    buffer.add(test_request);
...
TEST_F(McpHttpFilterChainFactoryTest, DISABLED_ProcessHttpRequest) {
  executeInDispatcher([this]() {
    ...
    std::string json_payload = R"({"jsonrpc":"2.0","id":1,"method":"initialize"})";
    std::string http_request = "POST /mcp/v1/sse HTTP/1.1\r\n"
                               "Host: localhost\r\n"
                               "Content-Type: application/json\r\n"
