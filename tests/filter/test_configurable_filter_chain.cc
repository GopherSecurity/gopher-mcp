 ...
 * @brief Comprehensive tests for configuration-driven filter chain functionality
 * 
...
class MockConfigurableFilterFactory : public FilterFactory {
 public:
  ...
  const std::vector<std::shared_ptr<MockConfigurableFilter>>& getCreatedFilters() const {
...
class MockFilterManager : public network::FilterManager {
 public:
  ...
  const std::vector<network::FilterSharedPtr>& getFilters() const { return filters_; }
  const std::vector<std::string>& getFilterNames() const { return filter_names_; }
...
class ConfigFileFixture {
 public:
  ...
  std::string writeJsonConfig(const std::string& name, const json::JsonValue& config) {
  ...
  // std::string writeYamlConfig(const std::string& name, const std::string& yaml)
...
class ConfigurableFilterChainTest : public ::testing::Test {
 protected:
  ...
  std::shared_ptr<MockConfigurableFilterFactory> getFactory(const std::string& type) {
            ...
            .add(json::JsonObjectBuilder()
                ...
                .add("filters", json::JsonArrayBuilder()
                    .add(json::JsonObjectBuilder()
                        ...
                        .add("config", json::JsonObjectBuilder()
                            .add("mode", "server")
            ...
            .add(json::JsonObjectBuilder()
                ...
                .add("filters", json::JsonArrayBuilder()
                    ...
                    .add(json::JsonObjectBuilder()
                        ...
                        .add("config", json::JsonObjectBuilder()
                            .add("max_requests_per_second", 100)
                            .add("burst_size", 200)
                            .build())
                        .build())
                    .add(json::JsonObjectBuilder()
  ...
  // YAML config creation methods removed - will be implemented when FileConfigSource is exposed

  std::unique_ptr<ConfigFileFixture> config_fixture_;
  std::map<std::string, std::shared_ptr<MockConfigurableFilterFactory>> factories_;
};
...
TEST_F(ConfigurableFilterChainTest, HttpSseJsonRpcChain) {
  ...
  for (const auto& filter : filters) {
    auto mock_filter = std::dynamic_pointer_cast<MockConfigurableFilter>(filter);
...
TEST_F(ConfigurableFilterChainTest, StdioJsonRpcOnly) {
  ...
  const auto& filters = filter_manager.getFilters();
  auto json_rpc_filter = std::dynamic_pointer_cast<MockConfigurableFilter>(filters[0]);
  
  EXPECT_EQ(json_rpc_filter->getName(), "");  // Mock filter doesn't set name from config
          ...
          .add(json::JsonObjectBuilder()
              ...
              .add("filters", json::JsonArrayBuilder()
                  ...
                  .add(json::JsonObjectBuilder()
                      .add("type", "json_rpc")
                      .add("name", "protocol")
                      .add("config", json::JsonValue::object())
                      .build())
                  .build())
              .build())
          .build())
...
TEST_F(ConfigurableFilterChainTest, FilterChainErrorPropagation) {
  ...
  auto metrics_filter = std::dynamic_pointer_cast<MockConfigurableFilter>(filters[0]);
  auto rate_limit_filter = std::dynamic_pointer_cast<MockConfigurableFilter>(filters[1]);
