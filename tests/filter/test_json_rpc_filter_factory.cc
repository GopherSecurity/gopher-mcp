...
TEST_F(JsonRpcFilterChainFactoryTest, DirectCallbacksAdapter) {
  executeInDispatcher([this]() {
    ...
    EXPECT_CALL(*message_callbacks_, onError(_))
        .WillOnce([](const Error& err) {
...
TEST_F(JsonRpcFilterChainFactoryTest, FilterLifetimeManagement) {
  executeInDispatcher([this]() {
    ...
    
    {
      ...
      JsonRpcFilterChainFactory factory(
          *dispatcher_,
          *message_callbacks_,
          true);  // use framing
