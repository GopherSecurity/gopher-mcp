...
class MockValidationCallbacks : public RequestValidationFilter::ValidationCallbacks {
public:
  MOCK_METHOD(void, onRequestValidated, (const std::string& method), (override));
  MOCK_METHOD(void, onRequestRejected, 
              (const std::string& method, const std::string& reason), 
              (override));
  MOCK_METHOD(void, onRateLimitExceeded, (const std::string& method), (override));
};
...
TEST_F(RequestValidationFilterTest, InvalidProtocolVersion) {
  ...
  EXPECT_CALL(*callbacks_, onRequestRejected("test.method", 
              HasSubstr("Invalid protocol version"))).Times(1);
...
TEST_F(RequestValidationFilterTest, BlockedMethod) {
  ...
  EXPECT_CALL(*callbacks_, onRequestRejected("admin/delete", 
              HasSubstr("Method is blocked"))).Times(1);
  EXPECT_CALL(*next_callbacks_, onRequest(_)).Times(0);
...
TEST_F(RequestValidationFilterTest, MethodNotInWhitelist) {
  ...
  EXPECT_CALL(*callbacks_, onRequestRejected("unknown.method", 
              HasSubstr("Method not in whitelist"))).Times(1);
  EXPECT_CALL(*next_callbacks_, onRequest(_)).Times(0);
...
TEST_F(RequestValidationFilterTest, NotificationValidation) {
  ...
  EXPECT_CALL(*callbacks_, onRequestRejected("blocked.notification", _)).Times(1);
