...
protected:
  ...
  void setupServer() {
    ...
    auto address = network::Address::anyAddress(network::Address::IpVersion::v4, 0);
  ...
  void setupClient() {
    ...
    // Connect to server
    auto server_address = network::Address::parseInternetAddress(
        "127.0.0.1", server_port_);
  ...
  public:
    ...
    void onMessageComplete() override {
      test_.server_request_complete_ = true;
    }
...
TEST_F(HttpSseFilterChainTest, HttpRequestResponse) {
  ...
  // Wait for response
  ASSERT_TRUE(waitForCondition([this]() {
    return server_request_complete_;
  }));
