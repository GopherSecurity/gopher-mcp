 ...
 * ----------------------
 * SSL transport socket is inherently async and complex, requiring multiple test levels:
 ...
 * - Thread safety assertions may trigger if callbacks aren't properly dispatched
...
public:
  ...
  void post(std::function<void()> callback) override {
    if (callback) callback();  // Execute immediately
  }
  ...
  ::mcp::event::TimerPtr createScaledTimer(::mcp::event::ScaledTimerMinimum minimum, 
                                           ::mcp::event::TimerCb cb) override {
    return createTimer(cb);
  }
  ...
  ::mcp::event::SignalEventPtr listenForSignal(int signal_num, 
                                               ::mcp::event::SignalCb cb) override {
    return nullptr;
  }
  
  ::mcp::event::FileEventPtr createFileEvent(int fd, ::mcp::event::FileReadyCb cb, 
  ...
  void pushTrackedObject(const ::mcp::event::ScopeTrackedObject* object) override {}
  void popTrackedObject(const ::mcp::event::ScopeTrackedObject* expected_object) override {}
...
TEST_F(SslTransportSocketUnitTest, ConnectOperation) {
  ...
  network::IoHandlePtr io_handle = std::make_unique<network::IoSocketHandleImpl>(0);
  network::ConnectionSocketImpl dummy_socket(std::move(io_handle), nullptr, nullptr);
...
public:
  ...
  network::Connection& connection() override {
    ...
    static network::ConnectionImpl dummy_conn(dispatcher, std::move(dummy_socket), 
                                              std::move(dummy_transport), false);
    return dummy_conn;
  }
...
TEST_F(SslTransportSocketIntegrationTest, MultipleCallbacks) {
  ...
  network::IoHandlePtr io_handle1 = std::make_unique<network::IoSocketHandleImpl>(0);
  network::ConnectionSocketImpl dummy_socket1(std::move(io_handle1), nullptr, nullptr);
