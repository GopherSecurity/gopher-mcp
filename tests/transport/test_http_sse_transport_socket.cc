/**
 * Comprehensive tests for HTTP+SSE transport socket - the backbone of MCP transport
 * 
 * These tests cover:
...
class MockIoHandle : public network::IoHandle {
 public:
  ...
  MOCK_METHOD(IoResult<int>, bind, (const network::Address::InstanceConstSharedPtr& address), (override));
  MOCK_METHOD(IoResult<int>, listen, (int backlog), (override));
  MOCK_METHOD(IoResult<network::IoHandlePtr>, accept, (), (override));
  MOCK_METHOD(IoResult<int>, connect, (const network::Address::InstanceConstSharedPtr& address), (override));
  ...
  MOCK_METHOD(IoResult<network::Address::InstanceConstSharedPtr>, localAddress, (), (const, override));
  MOCK_METHOD(IoResult<network::Address::InstanceConstSharedPtr>, peerAddress, (), (const, override));
...
class HttpSseTransportTestBase : public ::testing::Test {
 protected:
   ...
   */
  std::string createHttpResponse(int status_code, const std::string& reason,
...
class HttpSseTransportRealIoTest : public mcp::test::RealIoTestBase {
 protected:
  ...
  void createMockHttpServer(uint16_t& port) {
    executeInDispatcher([this, &port]() {
      ...
      server_handle_->initializeFileEvent(
          *dispatcher_,
          [this](uint32_t events) { onServerAccept(events); },
  ...
  void sendSseResponse(network::IoHandle* client) {
    ...
    std::string event = "data: {\"type\":\"welcome\",\"message\":\"Connected to SSE\"}\n\n";
    buffer.add(event);
    client->write(buffer);
  }
...
TEST_F(HttpSseTransportHttpTest, HttpResponseParsing) {
  ...
  std::string response = createHttpResponse(200, "OK", {
      {"Content-Type", "text/event-stream"},
      {"Cache-Control", "no-cache"},
      {"Connection", "keep-alive"}
  });
...
TEST_F(HttpSseTransportHttpTest, HttpErrorStatusHandling) {
  ...
  std::string response = createHttpResponse(404, "Not Found", {
...
TEST_F(HttpSseTransportSseTest, MultiLineSseEvent) {
  ...
  std::string sse_data = createSseEvent("message", 
      "Line 1\nLine 2\nLine 3");
...
TEST_F(HttpSseTransportSseTest, SseCommentHandling) {
  ...
  std::string sse_data = ": This is a comment\n"
                         ": Keepalive ping\n"
                         "data: actual data\n\n";
...
TEST_F(HttpSseTransportSseTest, SseRetryDirective) {
  ...
  std::string sse_data = "retry: 3000\n"
...
TEST_F(HttpSseTransportSseTest, SseUtf8BomHandling) {
  ...
  std::string sse_data = "\xEF\xBB\xBF"  // UTF-8 BOM
                         "data: test with BOM\n\n";
...
TEST_F(HttpSseTransportSseTest, MultipleSseEventsInBuffer) {
  ...
  std::string sse_data = 
      createSseEvent("message", "Event 1") +
...
TEST_F(HttpSseTransportRealIoTest, ClientServerCommunication) {
  ...
  executeInDispatcher([this, server_port, &test_done]() {
    ...
    EXPECT_TRUE(connect_result.ok() || errno == EINPROGRESS || errno == EWOULDBLOCK);
