 ...
 * @brief Comprehensive tests for TCP echo server implementation using MCP abstractions
 * 
...
public:
  ...
  std::string receiveData() {
    if (received_data_.empty()) return "";
...
protected:
  ...
  network::ListenerConfig createListenerConfig(
      const std::string& name, uint16_t port) {
    network::ListenerConfig config;
    config.name = name;
    config.address = network::Address::anyAddress(
        network::Address::IpVersion::v4, port);
...
TEST_F(TcpEchoServerBasicTest, ListenerConfiguration) {
  ...
  config2.address = network::Address::anyAddress(network::Address::IpVersion::v4, 0);
...
TEST_F(TcpEchoServerBasicTest, ListenerCallbacksInterface) {
  ...
  if (socket_result.ok()) {
    ...
    auto local_addr = network::Address::anyAddress(network::Address::IpVersion::v4, 0);
...
TEST_F(TcpEchoServerBasicTest, ServerConnectionCreation) {
  ...
  auto io_handle = socket_interface_->ioHandleForFd(*socket_result.value, false);
  auto local_addr = network::Address::anyAddress(network::Address::IpVersion::v4, 8080);
  auto remote_addr = network::Address::parseInternetAddress("127.0.0.1", 12345);
  ...
  // Note: We can't actually create a server connection without dispatcher running
  // but we've tested all the components needed
...
TEST_F(TcpEchoServerBasicTest, MultipleListenerConfigs) {
  ...
  for (int port = 8080; port < 8085; port++) {
    ...
    addresses.push_back(network::Address::anyAddress(network::Address::IpVersion::v4, port));
  }
...
TEST_F(TcpEchoServerBasicTest, ConnectionAcceptSimulation) {
  ...
  if (socket_result.ok()) {
    auto io_handle = socket_interface_->ioHandleForFd(*socket_result.value, false);
    auto local_addr = network::Address::anyAddress(network::Address::IpVersion::v4, 8080);
    auto remote_addr = network::Address::parseInternetAddress("127.0.0.1", 54321);
...
TEST_F(TcpEchoServerBasicTest, BindErrorHandling) {
  ...
  auto invalid_addr = network::Address::parseInternetAddress("256.256.256.256", 8080);
