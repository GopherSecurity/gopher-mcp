...
TEST_F(StdioEchoServerBasicTest, HandleNotification) {
  ...
  // Send a notification
  std::string notification = R"({"jsonrpc":"2.0","method":"test.event","params":{"data":"test"}})" "\n";
  ...
  if (n > 0) {
    ...
    EXPECT_TRUE(response.find("\"method\":\"echo/test.event\"") != std::string::npos);
...
TEST_F(StdioEchoServerBasicTest, HandleShutdown) {
  ...
  // The server might exit with SIGABRT due to thread cleanup issues, which is acceptable
  EXPECT_TRUE(WIFEXITED(status) || WIFSIGNALED(status));
