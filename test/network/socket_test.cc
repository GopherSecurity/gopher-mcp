...
public:
  ...
  MOCK_METHOD(IoResult<int>, setSocketOption,
              (int level, int optname, const void* optval, socklen_t optlen),
              (override));
  
  MOCK_METHOD(IoResult<int>, getSocketOption,
...
TEST_F(SocketTest, LocalAddressRestore) {
  ...
  EXPECT_EQ(socket.connectionInfoProvider().localAddress()->asString(), "127.0.0.1:8081");
  EXPECT_FALSE(setter.localAddressRestored());
  
  // Restore local address
  setter.restoreLocalAddress(local_addr);
  EXPECT_EQ(socket.connectionInfoProvider().localAddress()->asString(), "127.0.0.1:8080");
  EXPECT_TRUE(setter.localAddressRestored());
}

TEST_F(SocketTest, IoHandleOperations) {
  auto mock_handle = std::make_unique<MockIoHandle>();
  MockIoHandle* handle_ptr = mock_handle.get();
  ...
  // Test close
  EXPECT_CALL(*handle_ptr, close()).Times(1);
  socket.close();
}
...
TEST_F(SocketTest, ConnectOperation) {
  ...
  EXPECT_EQ(socket.connectionInfoProvider().remoteAddress()->asString(), "127.0.0.1:9090");
}
...
TEST_F(SocketTest, AddOptions) {
  ...
  options->push_back(std::make_shared<BoolSocketOption>(SOCKET_SO_REUSEPORT, true));
  options->push_back(std::make_shared<IntSocketOption>(SOCKET_SO_RCVBUF, 65536));
...
TEST_F(SocketTest, Duplicate) {
  ...
  auto dup_handle = std::make_unique<MockIoHandle>();
  EXPECT_CALL(*dup_handle, fd()).WillRepeatedly(Return(11));
  ...
  EXPECT_EQ(dup_socket->connectionInfoProvider().localAddress()->asString(), "127.0.0.1:8080");
  EXPECT_EQ(dup_socket->connectionInfoProvider().remoteAddress()->asString(), "127.0.0.1:9090");
...
TEST_F(SocketTest, ConnectionSocket) {
  ...
  ConnectionSocketImpl conn_socket(std::move(mock_handle), local_addr, remote_addr);
  ...
  EXPECT_EQ(conn_socket.detectedCloseType(), ConnectionSocket::DetectedCloseType::Normal);
...
TEST_F(SocketTest, ListenSocket) {
  ...
  EXPECT_GE(listen_socket.options()->size(), 2);  // At least reuse_address and reuse_port
}
...
TEST_F(SocketTest, CreateConnectionSocket) {
  ...
  auto socket = createConnectionSocket(Address::Type::Ip, remote_addr, local_addr, options);
...
TEST_F(SocketTest, ApplySocketOptions) {
  ...
  EXPECT_CALL(*handle_ptr, setSocketOption(SOL_SOCKET, SO_REUSEADDR, _, _))
      .WillOnce(Return(IoResult<int>::success(0)));
  ...
  options->push_back(std::make_shared<BoolSocketOption>(SOCKET_TCP_NODELAY, true));
...
TEST_F(SocketTest, ErrorHandling) {
  ...
  auto bind_result = socket.bind(Address::loopbackAddress(Address::IpVersion::v4, 8080));
  EXPECT_FALSE(bind_result.ok());
  EXPECT_EQ(bind_result.error_code.value_or(0), EBADF);
  
  auto listen_result = socket.listen(128);
  EXPECT_FALSE(listen_result.ok());
  EXPECT_EQ(listen_result.error_code.value_or(0), EBADF);
  
  auto connect_result = socket.connect(Address::loopbackAddress(Address::IpVersion::v4, 9090));
  EXPECT_FALSE(connect_result.ok());
  EXPECT_EQ(connect_result.error_code.value_or(0), EBADF);
  
  int value = 1;
  auto setsockopt_result = socket.setSocketOption(SOL_SOCKET, SO_REUSEADDR, &value, sizeof(value));
  EXPECT_FALSE(setsockopt_result.ok());
  EXPECT_EQ(setsockopt_result.error_code.value_or(0), EBADF);
  
  socklen_t len = sizeof(value);
  auto getsockopt_result = socket.getSocketOption(SOL_SOCKET, SO_REUSEADDR, &value, &len);
  EXPECT_FALSE(getsockopt_result.ok());
  EXPECT_EQ(getsockopt_result.error_code.value_or(0), EBADF);
  
  auto ioctl_result = socket.ioctl(FIONREAD, &value);
  EXPECT_FALSE(ioctl_result.ok());
  EXPECT_EQ(ioctl_result.error_code.value_or(0), EBADF);
  
  auto setblocking_result = socket.setBlocking(false);
  EXPECT_FALSE(setblocking_result.ok());
  EXPECT_EQ(setblocking_result.error_code.value_or(0), EBADF);
...
TEST_F(SocketTest, RealSocketIntegration) {
  ...
  auto conn_socket = createConnectionSocket(Address::Type::Ip, connect_addr, nullptr, options);
