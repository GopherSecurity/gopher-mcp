...
public class EncryptClientTransport extends CustomClientTransport {
    ...
    public EncryptClientTransport(BlockingQueue<McpSchema.JSONRPCMessage> inbound, BlockingQueue<McpSchema.JSONRPCMessage> outbound) {
        ...
        try {
            ...
            return super.sendMessage(encrypted)
                .onErrorMap(IllegalStateException.class, ex -> {
                    LOGGER.debug("[CLIENT] Transport closed during send operation: {}", ex.getMessage());
                    return ex;
                });
        } catch (Exception e) {
