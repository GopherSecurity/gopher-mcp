...
/**
 * Comprehensive unit tests for McpFilterBuffer Java wrapper.
 * Tests all buffer operations including creation, data manipulation,
 * reservations, zero-copy operations, and edge cases.
 */
...
public class McpFilterBufferTest {
    ...
    public void testCreateFromFragment() {
        ...
        assertEquals(testData.length(), buffer.length(handle), "Buffer length should match fragment data");
    }
    ...
    public void testCloneBuffer() {
        ...
        assertEquals(buffer.length(original), buffer.length(cloned), "Cloned buffer should have same length");
    }
    ...
    public void testCreateCowBuffer() {
        ...
        assertEquals(buffer.length(original), buffer.length(cow), "COW buffer should have same initial length");
    }
    ...
    public void testAddString() {
        ...
        assertEquals(testString.length(), buffer.length(handle), "Buffer length should match string length");
    }
    ...
    public void testAddBuffer() {
        ...
        assertEquals(buffer.length(source), buffer.length(dest), "Destination should contain source data");
    }
    ...
    public void testPrepend() {
        ...
        assertEquals("Hello World", new String(peeked, StandardCharsets.UTF_8), "Content should be in correct order");
    }
    ...
    public void testDrain() {
        ...
        assertEquals(initialLength - 5, buffer.length(handle), "Length should be reduced by drain amount");
    }
    ...
    public void testReserve() {
        ...
        assertTrue(reservation.getCapacity() >= 100, "Reservation capacity should be at least requested size");
    ...
    public void testGetContiguous() {
        ...
        assertEquals(testData.length(), contiguous.getLength(), "Contiguous length should match requested");
    }
    ...
    public void testLinearize() {
        ...
        assertEquals(testData.length(), linearized.remaining(), "Linearized buffer should have correct size");
    }
    ...
    public void testPeek() {
        ...
        assertEquals(testData, new String(peeked, StandardCharsets.UTF_8), "Peeked data should match original");

        // Verify peek doesn't consume data
        assertEquals(testData.length(), buffer.length(handle), "Buffer length should remain unchanged after peek");
    }
    ...
    public void testWatermarks() {
        ...
        assertFalse(buffer.aboveHighWatermark(handle), "Empty buffer should not be above high watermark");
        ...
        assertTrue(buffer.aboveHighWatermark(handle), "Buffer with 1500 bytes should be above high watermark");
        assertFalse(buffer.belowLowWatermark(handle), "Buffer with 1500 bytes should not be below low watermark");
        ...
        assertTrue(buffer.belowLowWatermark(handle), "Buffer with 50 bytes should be below low watermark");
        assertFalse(buffer.aboveHighWatermark(handle), "Buffer with 50 bytes should not be above high watermark");
    }
    ...
    public void testCreateWithString() {
        ...
        assertNotEquals(0, handle, "Buffer handle should not be zero");
        assertEquals(initialString.length(), buffer.length(handle), "Buffer should contain initial string");

        // Verify content
        byte[] peeked = buffer.peek(handle, 0, initialString.length());
        assertEquals(initialString, new String(peeked, StandardCharsets.UTF_8), "Buffer content should match initial string");
    }
    ...
    public void testMultipleOperations() {
        ...
        assertEquals("Zero First Second Third ", result, "All operations should be applied in correct order");
