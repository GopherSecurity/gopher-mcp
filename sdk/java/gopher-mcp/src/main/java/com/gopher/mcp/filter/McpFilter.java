...
/**
 * Java wrapper for the MCP Filter API.
 * Provides a high-level interface to the native MCP filter library through JNA.
 *
 * Features:
...
public class McpFilter implements AutoCloseable {
    ...
    public long create(long dispatcher, FilterConfig config) {
        ...
        nativeConfig.config_json = config.getConfigJson() != null ? Pointer.NULL : null; // TODO: Convert JSON
     ...
     */
    public int setCallbacks(long filter, FilterDataCallback onData, FilterWriteCallback onWrite,
                           FilterEventCallback onEvent, FilterMetadataCallback onMetadata,
                           FilterTrailersCallback onTrailers, Object userData) {
     ...
     */
    public long clientSendFiltered(FilterClientContext context, byte[] data,
                                   FilterCompletionCallback callback, Object userData) {
        ...
        if (callback != null) {
            nativeCallback = (result, user_data) -> {
                callback.onComplete(result);
                return;
            };
        }

        return lib.mcp_client_send_filtered(nativeContext, data, new NativeLong(data.length),
                                            nativeCallback, Pointer.NULL);
    }
     ...
     * @return MCP_OK on success
     */
    public int serverProcessFiltered(FilterServerContext context, long requestId,
                                     long requestBuffer, FilterRequestCallback callback,
                                     Object userData) {
        ...
        if (callback != null) {
            nativeCallback = (response_buffer, result, user_data) -> {
                callback.onRequest(response_buffer, result);
                return;
            };
        }

        return lib.mcp_server_process_filtered(nativeContext, requestId, requestBuffer,
                                               nativeCallback, Pointer.NULL);
    }
     ...
     * @return MCP_OK on success
     */
    public int postData(long filter, byte[] data, FilterPostCompletionCallback callback,
                        Object userData) {
        ...
        return lib.mcp_filter_post_data(filter, data, new NativeLong(data.length),
                                        nativeCallback, Pointer.NULL);
    }
    ...
    public long bufferPoolCreate(long bufferSize, long maxBuffers) {
        Pointer pool = lib.mcp_buffer_pool_create(new NativeLong(bufferSize), new NativeLong(maxBuffers));
