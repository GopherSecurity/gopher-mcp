...
/**
 * Java wrapper for the MCP Filter Chain API.
 * Provides one-to-one method mapping to McpFilterChainLibrary.
 *
 * This wrapper provides comprehensive filter chain composition and management,
...
public class McpFilterChain implements AutoCloseable {
     ...
     * Maps to: mcp_chain_builder_create_ex
     */
     ...
     * Maps to: mcp_chain_builder_add_node
     */
     ...
     * Maps to: mcp_chain_builder_add_conditional
     */
    public int chainBuilderAddConditional(long builder, FilterCondition condition, long filter) {
        ...
        nativeCondition.target_filter = condition.getTargetFilter() != 0 ?
            new Pointer(condition.getTargetFilter()) : null;
     ...
     * Maps to: mcp_chain_builder_add_parallel_group
     */
    public int chainBuilderAddParallelGroup(long builder, long[] filters) {
        return lib.mcp_chain_builder_add_parallel_group(
            new Pointer(builder),
            filters,
            new NativeLong(filters.length)
        );
    }

    /**
     * Set custom routing function
     * Maps to: mcp_chain_builder_set_router
     */
    public int chainBuilderSetRouter(long builder, McpFilterChainLibrary.MCP_ROUTING_FUNCTION_T router, long userData) {
     ...
     * Maps to: mcp_chain_set_filter_enabled
     */
     ...
     * Maps to: mcp_chain_set_event_callback
     */
    public int chainSetEventCallback(long chain, McpFilterChainLibrary.MCP_CHAIN_EVENT_CB callback, long userData) {
        Pointer userDataPtr = userData != 0 ? new Pointer(userData) : null;
     ...
     * Maps to: mcp_chain_create_from_json
     */
     ...
     * Maps to: mcp_chain_export_to_json
     */
    ...
    /**
     * Create chain router
     * Maps to: mcp_chain_router_create
     */
    public long chainRouterCreate(RouterConfig config) {
        ...
        nativeConfig.route_table = config.getRouteTable() != 0 ?
            new Pointer(config.getRouteTable()) : null;
        nativeConfig.custom_router_data = config.getCustomRouterData() != null ?
            new Pointer(Native.malloc(8)) : null;
    ...
    /**
     * Add route to router
     * Maps to: mcp_chain_router_add_route
     */
    public int chainRouterAddRoute(long router, McpFilterChainLibrary.MCP_FILTER_MATCH_CB condition, long chain) {
        return lib.mcp_chain_router_add_route(new Pointer(router), condition, chain);
    }

    /**
     * Route buffer through appropriate chain
     * Maps to: mcp_chain_router_route
     */
     ...
     * Maps to: mcp_chain_router_destroy
     */
     ...
     * Maps to: mcp_chain_pool_create
     */
    public long chainPoolCreate(long baseChain, long poolSize, ChainRoutingStrategy strategy) {
        Pointer pool = lib.mcp_chain_pool_create(baseChain, new NativeLong(poolSize), strategy.getValue());
        return Pointer.nativeValue(pool);
    }

    /**
     * Get next chain from pool
     * Maps to: mcp_chain_pool_get_next
     */
    ...
    /**
     * Get pool statistics
     * Maps to: mcp_chain_pool_get_stats
     */
    public int chainPoolGetStats(long pool, PointerByReference active,
                                  PointerByReference idle, LongByReference totalProcessed) {
        return lib.mcp_chain_pool_get_stats(new Pointer(pool), active, idle, totalProcessed);
    }

    /**
     * Destroy chain pool
     * Maps to: mcp_chain_pool_destroy
     */
     ...
     * Maps to: mcp_chain_reorder_filters
     */
     ...
     * Maps to: mcp_chain_set_trace_level
     */
