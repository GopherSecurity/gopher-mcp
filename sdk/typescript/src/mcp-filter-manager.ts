...
export interface FilterManagerConfig {
  ...
  // NEW: CApiFilter integration callbacks
  customCallbacks?: {
    onMessageReceived?: (message: JSONRPCMessage) => JSONRPCMessage | null;
    onMessageSent?: (message: JSONRPCMessage) => JSONRPCMessage | null;
  ...
  bufferPool?: BufferModule.BufferPoolConfig;
}
...
export class FilterManager {
  ...
  constructor(config: FilterManagerConfig = {}) {
    ...
    if (config.bufferPool) {
      this.bufferPool = BufferModule.createBufferPoolEx(config.bufferPool);
    }
  ...
  async process(message: JSONRPCMessage): Promise<JSONRPCMessage> {
    ...
    try {
      console.log(
        `🔍 [CApiFilter DEBUG] FilterManager.process called with message:`,
        JSON.stringify(message, null, 2)
      );

      // Check if BufferOwnership is available
      if (!BufferModule.BufferOwnership) {
        throw new Error("BufferOwnership is undefined - module loading issue");
      }
      if (BufferModule.BufferOwnership.SHARED === undefined) {
        throw new Error("BufferOwnership.SHARED is undefined");
      }

      // Convert message to buffer using filter-buffer.ts
      const messageStr = JSON.stringify(message);
      console.log(`🔍 [CApiFilter DEBUG] Message string: ${messageStr}`);
      const messageBuffer = BufferModule.createBufferFromString(
        messageStr,
        BufferModule.BufferOwnership.SHARED
      );
      console.log(`🔍 [CApiFilter DEBUG] Created buffer: ${messageBuffer}`);
      ...
      console.log(`🔍 [CApiFilter DEBUG] Result string: ${resultStr}`);
      const processedMessage = JSON.parse(resultStr);

      console.log(
        `🔍 [CApiFilter DEBUG] Final processed message:`,
  ...
  async processResponse(response: JSONRPCMessage): Promise<JSONRPCMessage> {
    ...
    try {
      // Debug: Check if BufferOwnership is available
      if (!BufferModule.BufferOwnership) {
        throw new Error("BufferOwnership is undefined - module loading issue");
      }
      if (BufferModule.BufferOwnership.SHARED === undefined) {
      ...
      const responseBuffer = BufferModule.createBufferFromString(
  ...
  private addFiltersToChain(
    ...
    // NEW: CApiFilter integration
    this.addCApiFiltersToChain(chainBuilder, config.customCallbacks);
  ...
  private async processThroughFilterChain(buffer: number): Promise<number> {
    ...
    return new Promise((resolve, reject) => {
      console.log(
        `🔍 [CApiFilter DEBUG] Calling postDataToFilter with filter chain: ${this.filterChain}`
      );
      postDataToFilter(
        this.filterChain,
        ...
        (result: any, _userData: any) => {
          console.log(
            `🔍 [CApiFilter DEBUG] postDataToFilter callback called with result: ${result}`
          );
          if (result === 0) {
            // 0 = MCP_OK
            console.log(
              `🔍 [CApiFilter DEBUG] Filter chain processing successful, returning buffer: ${buffer}`
            );
            resolve(buffer); // Return the processed buffer
          } else {
            console.log(
              `🔍 [CApiFilter DEBUG] Filter chain processing failed with result: ${result}`
            );
  ...
  /**
   * Add CApiFilter integration to chain
   */
  private addCApiFiltersToChain(
    chainBuilder: any,
    customCallbacks?: {
      onMessageReceived?: (message: JSONRPCMessage) => JSONRPCMessage | null;
      onMessageSent?: (message: JSONRPCMessage) => JSONRPCMessage | null;
    ...
    if (!customCallbacks) {
      console.log(
        `🔍 [CApiFilter DEBUG] No custom callbacks provided, skipping CApiFilter creation`
      );
      return;
    }

    console.log(
      `🔍 [CApiFilter DEBUG] Creating CApiFilter with custom callbacks:`,
      Object.keys(customCallbacks)
    );
    ...
    if (customCallbacks.onMessageReceived) {
      ...
      const filterNode: FilterNode = {
        filter: messageReceivedFilter,
        name: "message-received-filter",
    ...
    if (customCallbacks.onMessageSent) {
      ...
      const messageSentFilter = createCustomFilter(dispatcher, {
        onWrite: (buffer: number, _endStream: boolean, _userData: any) => {
          try {
            ...
            const processedMessage = customCallbacks.onMessageSent!(message);

            if (processedMessage) {
              // Update buffer with processed message
              updateBufferContent(buffer, JSON.stringify(processedMessage));
            }
      ...
      const filterNode: FilterNode = {
        ...
        name: "message-sent-filter",
        priority: 999, // Last in chain
        enabled: true,
        bypassOnError: false,
        config: {},
      };
