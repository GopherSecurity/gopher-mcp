...
/**
 * Token exchange result structure matching C API
 */
export interface TokenExchangeResultStruct {
  access_token: any;      // koffi C string pointer
  token_type: any;        // koffi C string pointer  
  expires_in: number;     // int64
  refresh_token: any;     // koffi C string pointer
  scope: any;             // koffi C string pointer
  error_code: number;
  error_description: any;  // koffi C string pointer
}

// Type definitions for FFI
const authTypes = {
  // Opaque handles (void pointers)
  mcp_auth_client_t: koffi.pointer('void'),
  mcp_auth_token_payload_t: koffi.pointer('void'),
  mcp_auth_validation_options_t: koffi.pointer('void'),
  mcp_auth_metadata_t: koffi.pointer('void'),
  ...
  mcp_auth_token_exchange_result_t: koffi.struct('mcp_auth_token_exchange_result_t', {
    access_token: 'char*',
    token_type: 'char*',
    expires_in: 'int64',
    refresh_token: 'char*',
...
export class AuthFFILibrary {
  ...
  private bindFunctions(): void {
    try {
      ...
      this.functions['mcp_auth_exchange_token'] = this.lib.func(
        'mcp_auth_exchange_token',
        authTypes.mcp_auth_token_exchange_result_t,
        [authTypes.mcp_auth_client_t, 'str', 'str', 'str', 'str']  // Try 'str' instead of 'const char*'
      );
      this.functions['mcp_auth_exchange_token_multi'] = this.lib.func(
        'mcp_auth_exchange_token_multi',
        authTypes.mcp_auth_error_t,
        [authTypes.mcp_auth_client_t, 'const char*', 'const char*', 
         koffi.out(koffi.pointer(authTypes.mcp_auth_token_exchange_result_t)), 
         koffi.out(koffi.pointer('size_t'))]
      );
      this.functions['mcp_auth_set_exchange_idps'] = this.lib.func(
        'mcp_auth_set_exchange_idps',
        authTypes.mcp_auth_error_t,
        [authTypes.mcp_auth_client_t, 'const char*']
      );
      this.functions['mcp_auth_free_exchange_result'] = this.lib.func(
        'mcp_auth_free_exchange_result',
        'void',
        [koffi.out(koffi.pointer(authTypes.mcp_auth_token_exchange_result_t))]
      );
