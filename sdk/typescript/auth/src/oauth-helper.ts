...
export class OAuthHelper {
  ...
  /**
   * Exchange OAuth authorization code for access token (NOT external IDP exchange)
   * This is part of the standard OAuth flow, not IDP token exchange
   */
  async exchangeToken(tokenRequest: any): Promise<any> {
    ...
    console.log(`Token will expire at: ${new Date(Date.now() + (data.expires_in || 300) * 1000).toISOString()}`);
    
    // Ensure expires_in is a number (MCP Inspector might expect this)
    if (data.expires_in) {
      data.expires_in = parseInt(data.expires_in, 10);
    }
