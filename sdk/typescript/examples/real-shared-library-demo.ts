...
async function demonstrateAdvancedBuffers() {
  ...
  try {
    ...
    logSuccess(`Added binary data: [${Array.from(binaryData).map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
...
async function demonstrateSharedLibraryIntegration() {
  ...
  try {
    ...
    if (filter) {
      ...
      // Get filter stats
      const stats = { bytes_processed: 0, packets_processed: 0, errors: 0, processing_time_us: 0, throughput_mbps: 0 };
...
async function demonstratePerformanceTesting() {
  ...
  try {
    ...
    logSuccess(`Created ${bufferCount} buffers in ${duration}ms (${(bufferCount / duration * 1000).toFixed(2)} buffers/sec)`);
    ...
    logSuccess(`Performed ${opCount} operations in ${opDuration}ms (${(opCount / opDuration * 1000).toFixed(2)} ops/sec)`);
