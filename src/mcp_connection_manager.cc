...
network::FilterStatus JsonRpcMessageFilter::onData(Buffer& data,
                                                   bool end_stream) {
  (void)end_stream;
...
network::FilterStatus JsonRpcMessageFilter::onWrite(Buffer& data,
                                                    bool end_stream) {
  (void)end_stream;
...
bool JsonRpcMessageFilter::parseMessage(const std::string& json_str) {
  try {
    ...
    // Debug output
    // std::cerr << "parseMessage: parsed JSON: " << json_val.toString() <<
    // std::endl;
