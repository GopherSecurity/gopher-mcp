...
void registerAllCoreFilters() {
  ...
  volatile void* refs[] = {
    http_codec_filter_registrar_ref,
    sse_codec_filter_registrar_ref,
  ...
  GOPHER_LOG(Debug, "Core filter registration complete, registry has {} context factories",
             factory_count);
  ...
  if (traditional_count > 0) {
    GOPHER_LOG(Debug, "Registry also has {} traditional factories", traditional_count);
  }
}
