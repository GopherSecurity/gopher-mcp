...
public:
  ...
  network::FilterStatus onNewConnection() override {
    ...
    if (circuit_breaker_) circuit_breaker_->onNewConnection();
    if (rate_limiter_) rate_limiter_->onNewConnection();
    if (metrics_collector_) metrics_collector_->onNewConnection();
    if (request_validator_) request_validator_->onNewConnection();
    if (backpressure_manager_) backpressure_manager_->onNewConnection();
  ...
  void initializeReadFilterCallbacks(network::ReadFilterCallbacks& callbacks) override {
    ...
    if (circuit_breaker_) circuit_breaker_->initializeReadFilterCallbacks(callbacks);
    if (rate_limiter_) rate_limiter_->initializeReadFilterCallbacks(callbacks);
    if (metrics_collector_) metrics_collector_->initializeReadFilterCallbacks(callbacks);
    if (request_validator_) request_validator_->initializeReadFilterCallbacks(callbacks);
    if (backpressure_manager_) backpressure_manager_->initializeReadFilterCallbacks(callbacks);
  ...
  void initializeWriteFilterCallbacks(network::WriteFilterCallbacks& callbacks) override {
    ...
    if (circuit_breaker_) circuit_breaker_->initializeWriteFilterCallbacks(callbacks);
    if (rate_limiter_) rate_limiter_->initializeWriteFilterCallbacks(callbacks);
    if (metrics_collector_) metrics_collector_->initializeWriteFilterCallbacks(callbacks);
    if (request_validator_) request_validator_->initializeWriteFilterCallbacks(callbacks);
    if (backpressure_manager_) backpressure_manager_->initializeWriteFilterCallbacks(callbacks);
  ...
  void onBody(const std::string& data, bool end_stream) override {
    if (is_sse_mode_) {
      sse_filter_->onData(reinterpret_cast<Buffer&>(const_cast<std::string&>(data)), end_stream);
    } else {
  ...
  void onRequestThrottled(const std::string& method, 
    ...
    std::cerr << "[Rate Limiter] Request throttled: " << method 
              << " (rate: " << current_rate << "/" << max_rate << ")" << std::endl;
  }
  ...
  void onValidationFailure(const jsonrpc::Request& request,
                          const std::string& reason) override {
    std::cerr << "[Validator] Request validation failed: " << reason << std::endl;
  }
