...
network::FilterStatus ProtocolDetectionFilter::onData(Buffer& data, bool end_stream) {
...
network::FilterStatus ProtocolDetectionFilter::onWrite(Buffer& data, bool end_stream) {
  // Pass through writes
  return network::FilterStatus::Continue;
}

ProtocolDetectionResult ProtocolDetectionFilter::detectProtocol(const Buffer& data) {
  ...
  if (isHttpRequest(data)) {
    ...
    const char* line_end = static_cast<const char*>(memchr(bytes, '\n', inspect_len));
    if (line_end) {
...
std::map<std::string, std::string> ProtocolDetectionFilter::extractHttpHeaders(const Buffer& data) {
  ...
  const char* headers_end = static_cast<const char*>(memmem(bytes, data_len, "\r\n\r\n", 4));
  if (!headers_end) {
  ...
  const char* line_start = static_cast<const char*>(memchr(bytes, '\n', data_len));
  if (!line_start) {
    return headers;
  }
  ...
  while (line_start < headers_end) {
    ...
    // Parse header line
    const char* colon = static_cast<const char*>(memchr(line_start, ':', line_len));
    if (colon) {
...
void ProtocolDetectionFilter::completeDetection(const ProtocolDetectionResult& result) {
...
network::FilterSharedPtr ProtocolRoutingFilterFactory::createProtocolDetectionFilter() {
...
std::vector<network::FilterSharedPtr> ProtocolRoutingFilterFactory::createHttpFilterChain(
...
std::vector<network::FilterSharedPtr> ProtocolRoutingFilterFactory::createMcpFilterChain(
    network::Connection& connection) {
  std::vector<network::FilterSharedPtr> filters;
...
void ProtocolRoutingFilterFactory::handleDetectedProtocol(
  ...
  // Note: In production, filters would be added through the connection's filter manager
  // The exact mechanism depends on the network layer implementation
  // For now, this demonstrates the architectural approach
}
...
bool TransparentProtocolHandler::isMcpHttpRequest(
  ...
  if (content_type != headers.end()) {
    if (content_type->second.find("application/json-rpc") != std::string::npos ||
