...
variant<Success, Error> StdioEchoTransport::connect(const std::string& endpoint) {
  if (!initialized_) {
...
variant<Success, Error> StdioEchoTransport::listen(const std::string& endpoint) {
...
variant<Success, Error> StdioEchoTransport::send(const std::string& data) {
  ...
  while (total_written < data.length()) {
    ssize_t written = write(stdout_fd_, 
                           data.c_str() + total_written,
    ...
    if (bytes_read > 0) {
      ...
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        ...
        if (callbacks_.onError) {
          callbacks_.onError(Error(errno, std::string("Read failed: ") + strerror(errno)));
        }
        break;
      }
    }
