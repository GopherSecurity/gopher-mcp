...
RequestContext::RequestContext(int id, const std::string& method, const Metadata& params)
...
int RequestManager::addRequest(const std::string& method, const Metadata& params) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void RequestManager::completeRequest(int id, const jsonrpc::Response& response) {
  std::lock_guard<std::mutex> lock(mutex_);
...
AdvancedEchoClient::~AdvancedEchoClient() {
  stop();
}
...
std::future<jsonrpc::Response> AdvancedEchoClient::sendRequest(
  ...
  if (holds_alternative<Error>(send_result)) {
    request_manager_->completeRequest(id, make<jsonrpc::Response>(id)
        .error(get<Error>(send_result))
        .build());
