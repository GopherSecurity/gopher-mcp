...
public:
  ...
  explicit Slice(BufferFragmentPtr fragment)
      : base_(const_cast<uint8_t*>(static_cast<const uint8_t*>(fragment->data()))),
  ...
  void reserve(RawSlice& slice, size_t size) {
    ...
    slice.mem_ = const_cast<void*>(static_cast<const void*>(base_ + reservable_));
    slice.len_ = size;
  }
  ...
  size_t prepend(const void* data, size_t size) {
    if (immutable_ || size > capacity_ - reservable_) return 0;
...
public:
  ...
  explicit Impl(BufferMemoryAccountPtr account) 
      : account_(account) {}
  ...
  void add(const void* data, size_t size) {
    ...
    while (size > 0) {
      ...
      if (!slices_.empty() && !slices_.back().isImmutable()) {
        ...
        slices_.updateSize(copied);  // Update byte size with actual copied amount
        src += copied;
        size -= copied;
        if (size == 0) break;
      }
  ...
  size_t getRawSlices(RawSlice* out, size_t max_slices) const {
    ...
    for (const auto& slice : slices_.slices_) {
      if (num_slices >= max_slices) break;
      if (slice.dataSize() > 0) {
  ...
  BufferSearchResult search(const void* pattern, size_t pattern_size, 
    ...
    for (const auto& slice : slices_.slices_) {
      ...
      size_t offset = (position < start_position) ? start_position - position : 0;
  ...
  bool startsWith(const void* pattern, size_t pattern_size) const {
    if (pattern_size == 0) return true;
    if (pattern_size > length()) return false;
    ...
    for (const auto& slice : slices_.slices_) {
      ...
      if (std::memcmp(slice.data(), pattern_bytes + compared, to_compare) != 0) {
        return false;
      }
      compared += to_compare;
...
void OwnedBuffer::add(const void* data, size_t size) {
  impl_->add(data, size);
}
...
void OwnedBuffer::drain(size_t size) {
  impl_->drain(size);
}
...
size_t OwnedBuffer::getRawSlices(ConstRawSlice* slices, size_t max_slices) const {
  return impl_->getRawSlices(slices, max_slices);
}

RawSlice OwnedBuffer::frontSlice() const {
  return impl_->frontSlice();
}

void* OwnedBuffer::linearize(size_t size) {
  return impl_->linearize(size);
}

BufferSearchResult OwnedBuffer::search(const void* pattern, size_t pattern_size, 
                                      size_t start_position) const {
...
size_t OwnedBuffer::length() const {
  return impl_->length();
}

size_t OwnedBuffer::sliceCount() const {
  return impl_->sliceCount();
}
...
std::string OwnedBuffer::toString() const {
  return impl_->toString();
}

// WatermarkBuffer implementation
void WatermarkBuffer::setWatermarks(uint32_t high_watermark, uint32_t overflow_multiplier) {
...
void WatermarkBuffer::checkWatermarks(size_t old_size) {
  ...
  if (old_size >= low_watermark_ && new_size < low_watermark_ && above_high_watermark_) {
