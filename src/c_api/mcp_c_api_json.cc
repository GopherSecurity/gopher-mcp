 ...
 * @brief JSON serialization/deserialization for MCP C API types using opaque handles
 *
 * This implementation uses the opaque handle API with RAII for safe memory management.
 * It provides JSON conversion for all MCP C API types through the public C API functions.
 */
...
MCP_API char* mcp_json_stringify(mcp_json_value_t json) MCP_NOEXCEPT {
    ...
    switch (type) {
        ...
        case MCP_JSON_TYPE_STRING:
            return alloc_string(std::string("\"") + safe_string(mcp_json_get_string(json)) + "\"");
...
MCP_API mcp_json_value_t mcp_request_id_to_json(const mcp_request_id_t* id) MCP_NOEXCEPT {
...
MCP_API mcp_request_id_t* mcp_request_id_from_json(mcp_json_value_t json) MCP_NOEXCEPT {
    if (!json) return nullptr;
...
MCP_API mcp_json_value_t mcp_progress_token_to_json(const mcp_progress_token_t* token) MCP_NOEXCEPT {
...
MCP_API mcp_progress_token_t* mcp_progress_token_from_json(mcp_json_value_t json) MCP_NOEXCEPT {
    if (!json) return nullptr;
...
MCP_API mcp_json_value_t mcp_content_block_to_json(const mcp_content_block_t* block) MCP_NOEXCEPT {
...
MCP_API mcp_content_block_t* mcp_content_block_from_json(mcp_json_value_t json) MCP_NOEXCEPT {
...
MCP_API mcp_json_value_t mcp_prompt_to_json(const mcp_prompt_t* prompt) MCP_NOEXCEPT {
    ...
    if (args_val && mcp_json_get_type(args_val) == MCP_JSON_TYPE_ARRAY) {
        ...
        for (size_t i = 0; i < arg_count; ++i) {
            ...
            if (arg && mcp_json_get_type(arg) == MCP_JSON_TYPE_OBJECT) {
                ...
                const char* arg_name = arg_name_val ? mcp_json_get_string(arg_name_val) : "";
                const char* arg_desc = arg_desc_val ? mcp_json_get_string(arg_desc_val) : "";
                mcp_bool_t required = arg_req_val ? mcp_json_get_bool(arg_req_val) : MCP_FALSE;
...
MCP_API mcp_json_value_t mcp_message_to_json(const mcp_message_t* message) MCP_NOEXCEPT {
...
MCP_API mcp_message_t* mcp_message_from_json(mcp_json_value_t json) MCP_NOEXCEPT {
    if (!json || mcp_json_get_type(json) != MCP_JSON_TYPE_OBJECT) return nullptr;
...
MCP_API mcp_json_value_t mcp_jsonrpc_error_to_json(const mcp_jsonrpc_error_t* error) MCP_NOEXCEPT {
...
MCP_API mcp_jsonrpc_error_t* mcp_jsonrpc_error_from_json(mcp_json_value_t json) MCP_NOEXCEPT {
    if (!json || mcp_json_get_type(json) != MCP_JSON_TYPE_OBJECT) return nullptr;
    ...
    int32_t code = code_val ? static_cast<int32_t>(mcp_json_get_number(code_val)) : 0;
...
MCP_API mcp_json_value_t mcp_jsonrpc_request_to_json(const mcp_jsonrpc_request_t* req) MCP_NOEXCEPT {
...
MCP_API mcp_json_value_t mcp_jsonrpc_response_to_json(const mcp_jsonrpc_response_t* resp) MCP_NOEXCEPT {
...
MCP_API mcp_jsonrpc_notification_t* mcp_jsonrpc_notification_from_json(mcp_json_value_t json) MCP_NOEXCEPT {
    ...
    mcp_jsonrpc_notification_t notification = mcp_jsonrpc_notification_create(jsonrpc, method);
...
MCP_API mcp_json_value_t mcp_initialize_request_to_json(const mcp_initialize_request_t* req) MCP_NOEXCEPT {
    if (!req) return nullptr;
...
MCP_API mcp_json_value_t mcp_initialize_result_to_json(const mcp_initialize_result_t* result) MCP_NOEXCEPT {
    ...
    mcp_server_capabilities_t caps = mcp_initialize_result_get_capabilities(*result);
    if (caps) {
...
MCP_API mcp_initialize_result_t* mcp_initialize_result_from_json(mcp_json_value_t json) MCP_NOEXCEPT {
    ...
    mcp_initialize_result_t init_result = mcp_initialize_result_create(protocol_version);
...
MCP_API mcp_json_value_t mcp_role_to_json(mcp_role_t role) MCP_NOEXCEPT {
    const char* role_str = "";
    switch (role) {
        case MCP_ROLE_USER: role_str = "user"; break;
        case MCP_ROLE_ASSISTANT: role_str = "assistant"; break;
...
MCP_API mcp_role_t mcp_role_from_json(mcp_json_value_t json) MCP_NOEXCEPT {
    ...
    if (!role_str) return MCP_ROLE_USER;
...
MCP_API mcp_resource_t* mcp_resource_from_json(mcp_json_value_t json) MCP_NOEXCEPT {
    ...
    if (desc_val) {
        ...
        if (desc) mcp_resource_set_description(resource, desc);
    }
    
    mcp_json_value_t mime_val = mcp_json_object_get(json, "mimeType");
    if (mime_val) {
        const char* mime = mcp_json_get_string(mime_val);
        if (mime) mcp_resource_set_mime_type(resource, mime);
    }
...
MCP_API mcp_implementation_t* mcp_implementation_from_json(mcp_json_value_t json) MCP_NOEXCEPT {
    ...
    const char* name = name_val ? mcp_json_get_string(name_val) : "";
    const char* version = version_val ? mcp_json_get_string(version_val) : "1.0.0";
...
MCP_API mcp_json_value_t mcp_server_capabilities_to_json(const mcp_server_capabilities_t* caps) MCP_NOEXCEPT {
    ...
    if (mcp_server_capabilities_has_resources(*caps)) {
        ...
        mcp_json_object_set(resources_obj, "listChanged", mcp_json_create_bool(true));
...
MCP_API mcp_json_value_t mcp_string_to_json(mcp_string_t str) MCP_NOEXCEPT {
    if (!str.data) return mcp_json_create_null();
