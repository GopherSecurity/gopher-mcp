...
struct mcp_json_value_handle {
    ...
    explicit mcp_json_value_handle(mcp::json::JsonValue&& v) : value(std::move(v)) {}
};
...
extern "C" mcp_json_value_t mcp_content_block_to_json(const mcp_content_block_t* block) {
    ...
    try {
        ...
        switch (block->type) {
            case MCP_CONTENT_TEXT: {
                ...
                if (block->text.annotations.has_value) {
                    ...
                    if (block->text.annotations.value.audience_count > 0) {
                        ...
                        for (size_t i = 0; i < block->text.annotations.value.audience_count; ++i) {
                            audience.push_back(block->text.annotations.value.audience[i] == MCP_ROLE_USER ? "user" : "assistant");
                        }
            ...
            case MCP_CONTENT_RESOURCE: {
                ...
                if (block->resource.resource.description) {
                    resource["description"] = safe_string(block->resource.resource.description);
                }
                if (block->resource.resource.mime_type) {
                    resource["mimeType"] = safe_string(block->resource.resource.mime_type);
                }
            ...
            case MCP_CONTENT_EMBEDDED: {
                ...
                if (block->embedded.resource.description) {
                    resource["description"] = safe_string(block->embedded.resource.description);
                }
                if (block->embedded.resource.mime_type) {
                    resource["mimeType"] = safe_string(block->embedded.resource.mime_type);
                }
                json["resource"] = resource;
                ...
                if (block->embedded.content_count > 0 && block->embedded.content) {
                    ...
                    for (size_t i = 0; i < block->embedded.content_count; ++i) {
                        mcp_json_value_t nested = mcp_content_block_to_json(&block->embedded.content[i]);
                        if (nested) {
        ...
        return reinterpret_cast<mcp_json_value_t>(new mcp_json_value_handle(std::move(json)));
    } catch (...) {
        return nullptr;
    }
}
        ...
        if (type_str == "text") {
            ...
            if (res_it != value.end() && res_it->is_object()) {
                ...
                if (uri_it != res_it->end() && uri_it->is_string()) {
                    block->resource.resource.uri = alloc_string(uri_it->get<std::string>());
                }
                auto name_it = res_it->find("name");
                if (name_it != res_it->end() && name_it->is_string()) {
                    block->resource.resource.name = alloc_string(name_it->get<std::string>());
                }
                auto desc_it = res_it->find("description");
                if (desc_it != res_it->end() && desc_it->is_string()) {
                    block->resource.resource.description = alloc_string(desc_it->get<std::string>());
                }
                auto mime_it = res_it->find("mimeType");
                if (mime_it != res_it->end() && mime_it->is_string()) {
                    block->resource.resource.mime_type = alloc_string(mime_it->get<std::string>());
                }
            }
        }
            ...
            if (res_it != value.end() && res_it->is_object()) {
                auto uri_it = res_it->find("uri");
                if (uri_it != res_it->end() && uri_it->is_string()) {
                    block->embedded.resource.uri = alloc_string(uri_it->get<std::string>());
                }
                auto name_it = res_it->find("name");
                if (name_it != res_it->end() && name_it->is_string()) {
                    block->embedded.resource.name = alloc_string(name_it->get<std::string>());
                }
                auto desc_it = res_it->find("description");
                if (desc_it != res_it->end() && desc_it->is_string()) {
                    block->embedded.resource.description = alloc_string(desc_it->get<std::string>());
                }
                auto mime_it = res_it->find("mimeType");
                if (mime_it != res_it->end() && mime_it->is_string()) {
                    block->embedded.resource.mime_type = alloc_string(mime_it->get<std::string>());
                }
            }
                ...
                for (size_t i = 0; i < count; ++i) {
                    ...
                    if (arg.is_object()) {
                        ...
                        if (arg_name != arg.end() && arg_name->is_string()) {
                            prompt->arguments[i].name = alloc_string(arg_name->get<std::string>());
                        }
                        auto arg_desc = arg.find("description");
                        if (arg_desc != arg.end() && arg_desc->is_string()) {
                            prompt->arguments[i].description = alloc_string(arg_desc->get<std::string>());
                        }
...
extern "C" mcp_json_value_t mcp_json_parse(const char* json_string) {
    ...
    try {
        ...
        return reinterpret_cast<mcp_json_value_t>(new mcp_json_value_handle(std::move(value)));
    } catch (...) {
        return nullptr;
    }
}
...
extern "C" mcp_json_value_t mcp_string_schema_to_json(const mcp_string_schema_t* schema) {
...
extern "C" mcp_json_value_t mcp_number_schema_to_json(const mcp_number_schema_t* schema) {
    if (!schema) return nullptr;
...
extern "C" mcp_json_value_t mcp_boolean_schema_to_json(const mcp_boolean_schema_t* schema) {
    if (!schema) return nullptr;
...
extern "C" mcp_json_value_t mcp_enum_schema_to_json(const mcp_enum_schema_t* schema) {
    if (!schema) return nullptr;
