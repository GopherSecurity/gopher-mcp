...
extern "C" void mcp_content_block_free(mcp_content_block_t* block) {
    ...
    switch (block->type) {
        case MCP_CONTENT_TEXT:
            ...
            if (block->text.annotations.has_value && block->text.annotations.value.audience) {
...
// Deep copy functions
extern "C" mcp_content_block_t* mcp_content_block_copy(const mcp_content_block_t* block) {
    if (!block) return nullptr;
    
    auto* copy = static_cast<mcp_content_block_t*>(std::calloc(1, sizeof(mcp_content_block_t)));
    ...
    switch (block->type) {
        case MCP_CONTENT_TEXT:
            ...
            if (block->text.annotations.has_value) {
                ...
                if (block->text.annotations.value.audience_count > 0 && 
                    ...
                    size_t size = block->text.annotations.value.audience_count * sizeof(mcp_role_t);
                    copy->text.annotations.value.audience = static_cast<mcp_role_t*>(std::malloc(size));
        ...
        case MCP_CONTENT_RESOURCE:
            ...
            copy->resource.resource.name = alloc_string(block->resource.resource.name);
            copy->resource.resource.description = alloc_string(block->resource.resource.description);
            copy->resource.resource.mime_type = alloc_string(block->resource.resource.mime_type);
            break;
            
        case MCP_CONTENT_EMBEDDED:
            copy->embedded.resource.uri = alloc_string(block->embedded.resource.uri);
            copy->embedded.resource.name = alloc_string(block->embedded.resource.name);
            copy->embedded.resource.description = alloc_string(block->embedded.resource.description);
            copy->embedded.resource.mime_type = alloc_string(block->embedded.resource.mime_type);
            
            if (block->embedded.content_count > 0 && block->embedded.content) {
                copy->embedded.content = static_cast<mcp_content_block_t*>(
                    std::calloc(block->embedded.content_count, sizeof(mcp_content_block_t)));
                if (copy->embedded.content) {
                    copy->embedded.content_count = block->embedded.content_count;
                    for (size_t i = 0; i < block->embedded.content_count; ++i) {
                        mcp_content_block_t* nested = mcp_content_block_copy(&block->embedded.content[i]);
                        if (nested) {
    ...
    if (prompt->argument_count > 0 && prompt->arguments) {
        ...
        if (copy->arguments) {
            ...
            for (size_t i = 0; i < prompt->argument_count; ++i) {
                ...
                copy->arguments[i].description = alloc_string(prompt->arguments[i].description);
...
extern "C" bool mcp_message_is_valid(const mcp_message_t* message) {
    return message && message->content && mcp_content_block_is_valid(message->content);
}
...
extern "C" bool mcp_request_id_equals(const mcp_request_id_t* a, const mcp_request_id_t* b) {
    if (!a || !b) return false;
...
// Array helpers
extern "C" mcp_content_block_array_t* mcp_content_block_array_create(size_t capacity) {
...
extern "C" bool mcp_content_block_array_append(
    mcp_content_block_array_t* array,
    mcp_content_block_t* block
) {
    if (!array || !block) return false;
    
    if (array->count >= array->capacity) {
        size_t new_capacity = array->capacity ? array->capacity * 2 : 4;
        auto* new_items = static_cast<mcp_content_block_t**>(
            std::realloc(array->items, new_capacity * sizeof(mcp_content_block_t*)));
...
extern "C" void mcp_memory_pool_free(mcp_memory_pool_t pool, void* ptr, size_t size) {
    if (!pool || !ptr) return;
...
extern "C" void mcp_string_free(char* str) {
    std::free(str);
}
...
extern "C" const char* mcp_logging_level_to_string(mcp_logging_level_t level) {
    switch (level) {
        case MCP_LOGGING_DEBUG: return "debug";
        case MCP_LOGGING_INFO: return "info";
        case MCP_LOGGING_NOTICE: return "notice";
        case MCP_LOGGING_WARNING: return "warning";
        case MCP_LOGGING_ERROR: return "error";
        case MCP_LOGGING_CRITICAL: return "critical";
        case MCP_LOGGING_ALERT: return "alert";
        case MCP_LOGGING_EMERGENCY: return "emergency";
        default: return "unknown";
    }
}

extern "C" mcp_logging_level_t mcp_logging_level_from_string(const char* str) {
    if (!str) return MCP_LOGGING_ERROR;
    
    if (std::strcmp(str, "debug") == 0) return MCP_LOGGING_DEBUG;
    if (std::strcmp(str, "info") == 0) return MCP_LOGGING_INFO;
    if (std::strcmp(str, "notice") == 0) return MCP_LOGGING_NOTICE;
    if (std::strcmp(str, "warning") == 0) return MCP_LOGGING_WARNING;
    if (std::strcmp(str, "error") == 0) return MCP_LOGGING_ERROR;
    if (std::strcmp(str, "critical") == 0) return MCP_LOGGING_CRITICAL;
    if (std::strcmp(str, "alert") == 0) return MCP_LOGGING_ALERT;
    if (std::strcmp(str, "emergency") == 0) return MCP_LOGGING_EMERGENCY;
