...
public:
    ...
    mcp_result_t add(void* resource, mcp_type_id_t type, mcp_guard_cleanup_fn cleanup_fn = nullptr) {
        ...
        if (options.max_resources > 0 && resources.size() >= options.max_resources) {
...
MCP_API mcp_guard_t mcp_guard_create_custom(void* handle,
                                            mcp_type_id_t type,
                                            mcp_guard_cleanup_fn cleanup) MCP_NOEXCEPT {
    if (!handle) return nullptr;
...
MCP_API mcp_transaction_t mcp_transaction_create(void) MCP_NOEXCEPT {
    try {
        ...
        return reinterpret_cast<mcp_transaction_t>(static_cast<uintptr_t>(txn_handle));
    } catch (...) {
        return nullptr;
    }
}
...
MCP_API mcp_result_t mcp_transaction_commit(mcp_transaction_t* txn) MCP_NOEXCEPT {
    if (!txn || !*txn) return MCP_ERROR_INVALID_ARGUMENT;
