...
extern "C" mcp_content_block_t* mcp_text_content_create(const char* text) {
  ...
  auto* text_content = static_cast<mcp_text_content_t*>(
...
extern "C" mcp_content_block_t* mcp_image_content_create(
    const char* data, const char* mime_type) {
  if (!data || !mime_type)
    return nullptr;
  ...
  auto* image_content = static_cast<mcp_image_content_t*>(
  ...
  if (!image_content->data.data || !image_content->mime_type.data) {
    if (image_content->data.data)
      std::free(const_cast<char*>(image_content->data.data));
    if (image_content->mime_type.data)
      std::free(const_cast<char*>(image_content->mime_type.data));
...
extern "C" mcp_content_block_t* mcp_audio_content_create(
  ...
  auto* audio_content = static_cast<mcp_audio_content_t*>(
  ...
  if (!audio_content->data.data || !audio_content->mime_type.data) {
    if (audio_content->data.data)
      std::free(const_cast<char*>(audio_content->data.data));
    if (audio_content->mime_type.data)
      std::free(const_cast<char*>(audio_content->mime_type.data));
...
extern "C" mcp_content_block_t* mcp_resource_content_create(
...
// Resource builders
extern "C" mcp_resource_t* mcp_resource_create(const char* uri,
                                               const char* name) {
  if (!uri || !name)
    return nullptr;
...
extern "C" mcp_resource_t* mcp_resource_with_details(const char* uri,
                                                     const char* name,
...
extern "C" mcp_tool_t* mcp_tool_with_description(const char* name,
                                                 const char* description) {
...
extern "C" mcp_tool_t* mcp_tool_with_schema(const char* name,
...
extern "C" mcp_tool_t* mcp_tool_complete(const char* name,
                                         const char* description,
                                         mcp_json_value_t schema) {
  auto* tool = mcp_tool_create(name);
  if (!tool)
...
extern "C" mcp_prompt_t* mcp_prompt_with_description(const char* name,
                                                     const char* description) {
    ...
    if (prompt->arguments) {
      for (size_t i = 0; i < argument_count; ++i) {
        ...
        prompt->arguments[i].description =
            alloc_string(arguments[i].description);
...
// Message builders
extern "C" mcp_message_t* mcp_message_create(mcp_role_t role,
...
extern "C" mcp_error_t* mcp_error_with_data(int code,
                                            const char* message,
...
extern "C" mcp_string_schema_t* mcp_string_schema_create() {
  return static_cast<mcp_string_schema_t*>(
      std::calloc(1, sizeof(mcp_string_schema_t)));
}
...
extern "C" mcp_number_schema_t* mcp_number_schema_create() {
  return static_cast<mcp_number_schema_t*>(
      std::calloc(1, sizeof(mcp_number_schema_t)));
}
...
extern "C" mcp_boolean_schema_t* mcp_boolean_schema_create() {
  return static_cast<mcp_boolean_schema_t*>(
      std::calloc(1, sizeof(mcp_boolean_schema_t)));
}
...
extern "C" mcp_enum_schema_t* mcp_enum_schema_create(const char** values,
                                                     size_t value_count) {
  if (!values || value_count == 0)
    return nullptr;

  auto* schema = static_cast<mcp_enum_schema_t*>(
      std::calloc(1, sizeof(mcp_enum_schema_t)));
  if (!schema)
    return nullptr;

  schema->values =
      static_cast<char**>(std::malloc(value_count * sizeof(char*)));
  if (!schema->values) {
    std::free(schema);
    return nullptr;
  }
...
extern "C" mcp_enum_schema_t* mcp_enum_schema_with_description(
    const char** values, size_t value_count, const char* description) {
...
// Request builders
extern "C" mcp_request_t* mcp_request_create(mcp_request_id_t id,
                                             const char* method) {
  if (!method)
    return nullptr;

  auto* request =
      static_cast<mcp_request_t*>(std::calloc(1, sizeof(mcp_request_t)));
  if (!request)
    return nullptr;
...
extern "C" mcp_request_t* mcp_request_with_params(mcp_request_id_t id,
                                                  const char* method,
...
// Response builders
extern "C" mcp_response_t* mcp_response_success(mcp_request_id_t id,
                                                mcp_json_value_t result) {
  auto* response =
      static_cast<mcp_response_t*>(std::calloc(1, sizeof(mcp_response_t)));
  if (!response)
    return nullptr;
...
extern "C" mcp_response_t* mcp_response_error(mcp_request_id_t id,
...
extern "C" mcp_notification_t* mcp_notification_create(const char* method) {
  ...
  auto* notification = static_cast<mcp_notification_t*>(
      std::calloc(1, sizeof(mcp_notification_t)));
  if (!notification)
    return nullptr;
...
extern "C" mcp_model_hint_t* mcp_model_hint_create(const char* name) {
  ...
  auto* hint =
      static_cast<mcp_model_hint_t*>(std::calloc(1, sizeof(mcp_model_hint_t)));
  if (!hint)
    return nullptr;
...
extern "C" mcp_model_preferences_t* mcp_model_preferences_create() {
  return static_cast<mcp_model_preferences_t*>(
      std::calloc(1, sizeof(mcp_model_preferences_t)));
}

extern "C" mcp_model_preferences_t* mcp_model_preferences_with_priorities(
    double cost_priority, double speed_priority, double intelligence_priority) {
