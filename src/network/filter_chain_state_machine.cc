...
const std::unordered_map<FilterChainState, std::unordered_set<FilterChainState>>&
getValidTransitions() {
  static const std::unordered_map<FilterChainState, std::unordered_set<FilterChainState>>
      transitions = {
          // Initial states
...
bool FilterChainStateMachine::pause() {
  ...
  return transitionTo(FilterChainState::Paused,
                     FilterChainEvent::PauseRequested,
                     "Pause requested");
}
...
bool FilterChainStateMachine::close() {
  ...
  if (buffered_bytes_ > 0) {
    transitionTo(FilterChainState::Flushing,
                FilterChainEvent::CloseRequested,
...
void FilterChainStateMachine::reorderFilters(const std::vector<std::string>& order) {
  assertInDispatcherThread();
...
FilterStatus FilterChainStateMachine::onWrite(Buffer& data, bool end_stream) {
  ...
  transitionTo(FilterChainState::ProcessingUpstream,
              FilterChainEvent::DataToSend,
              "Processing upstream data");
...
void FilterChainStateMachine::addFilter(FilterSharedPtr filter) {
  ...
  } else if (auto write_filter = std::dynamic_pointer_cast<WriteFilter>(filter)) {
...
bool FilterChainStateMachine::transitionTo(FilterChainState new_state,
  ...
  if (!isValidTransition(current, new_state)) {
    if (config_.error_callback) {
      std::stringstream ss;
      ss << "Invalid transition from " << static_cast<int>(current)
...
FilterStatus FilterChainStateMachine::iterateReadFilters(Buffer& data,
                                                        bool end_stream) {
  transitionTo(FilterChainState::IteratingRead,
              FilterChainEvent::DataReceived,
  ...
  if (status == FilterStatus::Continue) {
    ...
    transitionTo(FilterChainState::Active,
                FilterChainEvent::FilterContinue,
...
FilterStatus FilterChainStateMachine::iterateWriteFilters(Buffer& data,
                                                         bool end_stream) {
  transitionTo(FilterChainState::IteratingWrite,
              FilterChainEvent::DataToSend,
...
void FilterChainStateMachine::applyBackpressure() {
  if (!backpressure_applied_) {
    ...
    transitionTo(FilterChainState::AboveHighWatermark,
                FilterChainEvent::BufferHighWatermark,
...
void FilterChainStateMachine::releaseBackpressure() {
  if (backpressure_applied_) {
    ...
    transitionTo(FilterChainState::BelowLowWatermark,
                FilterChainEvent::BufferLowWatermark,
...
void FilterChainStateMachine::startDrainTimer() {
  ...
  drain_timer_ = dispatcher_.createTimer([this]() {
    transitionTo(FilterChainState::Closing,
                FilterChainEvent::DrainComplete,
                "Drain timeout");
  });
...
bool FilterChainStateMachine::attemptRecovery() {
  transitionTo(FilterChainState::Recovering,
              FilterChainEvent::FilterError,
              "Attempting recovery");
...
void FilterChainStateMachine::assertInDispatcherThread() const {
  ...
  dispatcher_thread_id_.compare_exchange_strong(expected, std::this_thread::get_id());
}
...
FilterChainBuilder& FilterChainBuilder::withWatermarks(uint32_t high, uint32_t low) {
  config_.high_watermark = high;
  config_.low_watermark = low;
  return *this;
}

FilterChainBuilder& FilterChainBuilder::withInitTimeout(std::chrono::milliseconds timeout) {
  config_.initialization_timeout = timeout;
  return *this;
}

FilterChainBuilder& FilterChainBuilder::withDrainTimeout(std::chrono::milliseconds timeout) {
  config_.drain_timeout = timeout;
  return *this;
}

FilterChainBuilder& FilterChainBuilder::withIdleTimeout(std::chrono::milliseconds timeout) {
...
FilterChainBuilder& FilterChainBuilder::withStateChangeCallback(
    std::function<void(FilterChainState, FilterChainState, const std::string&)> cb) {
...
FilterChainBuilder& FilterChainBuilder::addWriteFilter(const std::string& name,
                                                       FilterFactoryCb factory) {
