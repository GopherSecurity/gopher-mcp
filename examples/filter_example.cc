...
mcp_filter_status_t logging_filter_on_data(
...
mcp_filter_status_t rate_limit_filter_on_data(
    mcp_buffer_handle_t buffer,
    mcp_bool_t end_stream,
    void* user_data) {
...
mcp_filter_status_t compression_filter_on_write(
    mcp_buffer_handle_t buffer,
    mcp_bool_t end_stream,
    void* user_data) {
    ...
    std::cout << "[Compression] Compressed " << original_length 
              << " bytes to " << compressed_length << " bytes" << std::endl;
    
    return MCP_FILTER_CONTINUE;
}

// Error handler
void filter_error_handler(
    mcp_filter_t filter,
...
void example_basic_filter_chain() {
    ...
    mcp_filter_chain_builder_t builder = mcp_filter_chain_builder_create(dispatcher);
    ...
    mcp_filter_t rate_limit_filter = mcp_filter_create(dispatcher, &rate_limit_config);
    ...
    mcp_filter_chain_add_filter(builder, logging_filter, MCP_FILTER_POSITION_FIRST, 0);
    mcp_filter_chain_add_filter(builder, rate_limit_filter, MCP_FILTER_POSITION_LAST, 0);
    
    // Build chain
    mcp_filter_chain_t chain = mcp_filter_chain_build(builder);
    mcp_filter_chain_builder_destroy(builder);
    
    std::cout << "Filter chain created with logging and rate limiting" << std::endl;
...
void example_zero_copy_buffers() {
    ...
    if (result == MCP_OK) {
        std::cout << "Reserved " << slice.length << " bytes for writing" << std::endl;
...
void example_protocol_layers() {
    ...
    mcp_filter_chain_builder_t builder = mcp_filter_chain_builder_create(dispatcher);
    mcp_filter_chain_add_filter(builder, tcp_filter, MCP_FILTER_POSITION_FIRST, 0);
    mcp_filter_chain_add_filter(builder, tls_filter, MCP_FILTER_POSITION_LAST, 0);
    mcp_filter_chain_add_filter(builder, http_filter, MCP_FILTER_POSITION_LAST, 0);
    
    mcp_filter_chain_t chain = mcp_filter_chain_build(builder);
    std::cout << "Built protocol layer filter chain: TCP -> TLS -> HTTP" << std::endl;
...
void example_client_server_integration() {
    ...
    mcp_filter_chain_builder_t req_builder = mcp_filter_chain_builder_create(dispatcher);
    ...
    mcp_filter_chain_add_filter(req_builder, compression, MCP_FILTER_POSITION_LAST, 0);
    client_context.request_filters = mcp_filter_chain_build(req_builder);
    
    // Build response filter chain
    mcp_filter_chain_builder_t resp_builder = mcp_filter_chain_builder_create(dispatcher);
    ...
    mcp_filter_chain_add_filter(resp_builder, metrics, MCP_FILTER_POSITION_FIRST, 0);
    mcp_filter_chain_add_filter(resp_builder, logging, MCP_FILTER_POSITION_LAST, 0);
    ...
    mcp_request_id_t request_id = mcp_client_send_filtered(
        &client_context,
        reinterpret_cast<const uint8_t*>(request_data),
        strlen(request_data),
        completion_cb,
        nullptr);
