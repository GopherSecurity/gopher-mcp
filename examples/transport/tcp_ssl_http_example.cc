...
class ExampleTransportCallbacks : public TransportSocketCallbacks {
 public:
  ...
  void raiseEvent(ConnectionEvent event) override {
    std::cout << "Transport raised event: " << static_cast<int>(event) << std::endl;
  }
...
int main(int argc, char* argv[]) {
  ...
  // Default: Show configuration
    ...
    auto http_factory = std::make_unique<HttpsSseTransportFactory>(http_config, *dispatcher);
    std::cout << "http://api.example.com → " << http_factory->name() 
              << " (SSL: " << http_factory->implementsSecureTransport() << ")" << std::endl;
    
    HttpSseTransportSocketConfig https_config;
    https_config.endpoint_url = "https://api.example.com";
    auto https_factory = std::make_unique<HttpsSseTransportFactory>(https_config, *dispatcher);
    std::cout << "https://api.example.com → " << https_factory->name()
              << " (SSL: " << https_factory->implementsSecureTransport() << ")" << std::endl;
  ...
  auto factory = std::make_unique<HttpsSseTransportFactory>(config, *dispatcher);
  
  std::cout << "Factory name: " << factory->name() << std::endl;
  std::cout << "Implements secure transport: " 
            << (factory->implementsSecureTransport() ? "Yes" : "No") << std::endl;
  std::cout << "Supports ALPN: " 
            << (factory->supportsAlpn() ? "Yes" : "No") << std::endl;
  
  if (factory->implementsSecureTransport()) {
    std::cout << "Default SNI: " << factory->defaultServerNameIndication() << std::endl;
  }
  ...
  if (transport_socket) {
    ...
    std::cout << "Can flush close: " 
              << (transport_socket->canFlushClose() ? "Yes" : "No") << std::endl;
    ...
    std::cout << std::endl << "=== Example Usage (Pseudo-code) ===" << std::endl;
    ...
    std::cout << "transport_socket->setTransportSocketCallbacks(callbacks);" << std::endl;
    std::cout << "// Connect" << std::endl;
    std::cout << "auto result = transport_socket->connect(connection->socket());" << std::endl;
    std::cout << "// Send MCP request" << std::endl;
    std::cout << "transport_socket->doWrite(request_buffer, false);" << std::endl;
  ...
  std::cout << "The transport stack is configured and ready for use." << std::endl;
