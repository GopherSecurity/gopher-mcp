...
void demonstrateFeatures(McpClient& client, bool verbose) {
  ...
  // 1. Initialize protocol
  {
    ...
    try {
      ...
      std::cerr << "[DEMO] Protocol initialized: "
      ...
      if (verbose && init_result.serverInfo.has_value()) {
        std::cerr << "[DEMO] Server version: "
                  << init_result.serverInfo->version << std::endl;
      }
  ...
  // 2. List available resources
  {
    ...
    try {
      ...
      std::cerr << "[DEMO] Found " << list_result.resources.size()
                << " resources" << std::endl;
      ...
      for (const auto& resource : list_result.resources) {
        ...
        if (list_result.resources.size() > 0 && verbose) {
          ...
          try {
            ...
            std::cerr << "[DEMO] Read resource: "
                      << list_result.resources[0].name << std::endl;
          } catch (const std::exception& e) {
            std::cerr << "[ERROR] Failed to read resource: " << e.what()
                      << std::endl;
          }
      ...
      std::cerr << "[ERROR] Failed to list resources: " << e.what()
                << std::endl;
    }
  }
  ...
  // 3. List and call tools
  {
    ...
    try {
      ...
      std::cerr << "[DEMO] Found " << tools_result.tools.size() << " tools"
                << std::endl;
  ...
  // 4. Batch requests demonstration
  {
    ...
    std::cerr << "[DEMO] Sent " << futures.size() << " batch requests"
              << std::endl;
  ...
  if (verbose) {
    ...
    for (int i = 0; i < 10; ++i) {
      auto params =
          make<Metadata>().add("request_id", i).add("test", true).build();
