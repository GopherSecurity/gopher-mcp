...
public:
  ...
  void start(const std::string& cert_file,
    ...
    std::cout << "HTTPS MCP Server listening on " << listen_address << std::endl;
...
private:
  void registerHandlers() {
    ...
    server_->registerTool(example_tool,
...
int main(int argc, char* argv[]) {
  ...
  if (!is_server && !is_client) {
    ...
    std::cout << "  --url <url>       Server URL (default: https://localhost:8443)" << std::endl;
    std::cout << "  --cert <file>     Certificate file (server mode)" << std::endl;
    std::cout << "  --key <file>      Private key file (server mode)" << std::endl;
    std::cout << "  --ca <file>       CA certificate file (client mode)" << std::endl;
    std::cout << "  --no-verify       Disable SSL verification (client mode)" << std::endl;
    return 1;
  }
