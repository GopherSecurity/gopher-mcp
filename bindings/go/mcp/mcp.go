...
// extern void goDataCallback(mcp_connection_t, unsigned char*, size_t, void*);
// extern void goErrorCallback(int, char*, void*);
// extern void goTimerCallback(void*);
// extern void goRequestCallback(mcp_request_id_t, mcp_string_t, mcp_json_value_t, void*);
//
// static void c_connection_state_callback(mcp_connection_t conn, int old_state, int new_state, void* user_data) {
//     goConnectionStateCallback(conn, old_state, new_state, user_data);
// }
//
// static void c_data_callback(mcp_connection_t conn, const uint8_t* data, size_t length, void* user_data) {
//     goDataCallback(conn, (unsigned char*)data, length, user_data);
// }
//
// static void c_error_callback(int error, const char* message, void* user_data) {
...
const (
	OK                      Result = C.MCP_OK
	Error                   Result = C.MCP_ERROR
	ErrorInvalidArgument    Result = C.MCP_ERROR_INVALID_ARGUMENT
	ErrorOutOfMemory        Result = C.MCP_ERROR_OUT_OF_MEMORY
	ErrorNotConnected       Result = C.MCP_ERROR_NOT_CONNECTED
	ErrorTimeout            Result = C.MCP_ERROR_TIMEOUT
	ErrorCancelled          Result = C.MCP_ERROR_CANCELLED
	ErrorNotFound           Result = C.MCP_ERROR_NOT_FOUND
	ErrorAlreadyExists      Result = C.MCP_ERROR_ALREADY_EXISTS
	ErrorPermissionDenied   Result = C.MCP_ERROR_PERMISSION_DENIED
	ErrorResourceExhausted  Result = C.MCP_ERROR_RESOURCE_EXHAUSTED
	ErrorInvalidState       Result = C.MCP_ERROR_INVALID_STATE
	ErrorProtocol           Result = C.MCP_ERROR_PROTOCOL
	ErrorNotImplemented     Result = C.MCP_ERROR_NOT_IMPLEMENTED
	ErrorIO                 Result = C.MCP_ERROR_IO
	ErrorSSL                Result = C.MCP_ERROR_SSL
)
...
const (
	ConnectionStateConnecting    ConnectionState = C.MCP_CONNECTION_STATE_CONNECTING
	ConnectionStateConnected     ConnectionState = C.MCP_CONNECTION_STATE_CONNECTED
	ConnectionStateDisconnecting ConnectionState = C.MCP_CONNECTION_STATE_DISCONNECTING
	ConnectionStateDisconnected  ConnectionState = C.MCP_CONNECTION_STATE_DISCONNECTED
	ConnectionStateError         ConnectionState = C.MCP_CONNECTION_STATE_ERROR
)
...
const (
	TransportTCP     TransportType = C.MCP_TRANSPORT_TCP
	TransportSSL     TransportType = C.MCP_TRANSPORT_SSL
	TransportHTTPSSE TransportType = C.MCP_TRANSPORT_HTTP_SSE
	TransportStdio   TransportType = C.MCP_TRANSPORT_STDIO
	TransportPipe    TransportType = C.MCP_TRANSPORT_PIPE
)
...
func Init() (*Library, error) {
	result := C.mcp_init(nil) // Use default allocator
	if result != C.MCP_OK {
...
func (l *Library) Version() string {
	return C.GoString(C.mcp_get_version())
}

// GetLastError returns the last error message
func GetLastError() string {
	return C.GoString(C.mcp_get_last_error())
}

// CreateDispatcher creates a new event dispatcher
func (l *Library) CreateDispatcher() (*Dispatcher, error) {
	handle := C.mcp_dispatcher_create()
	if handle == nil {
		return nil, errors.New("failed to create dispatcher: " + GetLastError())
	}
...
func (d *Dispatcher) Run() error {
	...
	result := C.mcp_dispatcher_run(d.handle)
...
func (d *Dispatcher) RunTimeout(timeoutMs uint32) error {
	...
	result := C.mcp_dispatcher_run_timeout(d.handle, C.uint32_t(timeoutMs))
...
func (d *Dispatcher) CreateTimer(callback func()) (*Timer, error) {
	...
	timerID := C.mcp_dispatcher_create_timer(
		d.handle,
		C.mcp_timer_callback_t(C.c_timer_callback),
...
func (d *Dispatcher) destroy() {
	if d.handle != nil {
		C.mcp_dispatcher_destroy(d.handle)
		d.handle = nil
	}
}
...
func (t *Timer) Enable(timeoutMs uint32, repeat bool) error {
	result := C.mcp_dispatcher_enable_timer(
		t.dispatcher.handle,
		C.uint64_t(t.id),
...
func (t *Timer) Destroy() {
	...
	delete(t.dispatcher.timers, t.id)
	t.dispatcher.mu.Unlock()
}

// Connection represents a network connection
type Connection struct {
	handle     C.mcp_connection_t
	dispatcher *Dispatcher
...
func (d *Dispatcher) CreateConnection(transport TransportType) (*Connection, error) {
	handle := C.mcp_connection_create_client(d.handle, C.mcp_transport_type_t(transport))
	if handle == nil {
		return nil, errors.New("failed to create connection: " + GetLastError())
	}
	...
	runtime.SetFinalizer(conn, (*Connection).destroy)
	return conn, nil
}
...
func (c *Connection) SetCallbacks(callbacks ConnectionCallbacks) error {
	...
	c.callbackID = registerCallback(c)
	
	result := C.mcp_connection_set_callbacks(
		c.handle,
	...
	if result != C.MCP_OK {
		...
		return fmt.Errorf("failed to set callbacks: %d", result)
	}
	return nil
}
...
func (c *Connection) Connect() error {
	result := C.mcp_connection_connect(c.handle)
	if result != C.MCP_OK {
...
func (c *Connection) Close(flush bool) error {
	result := C.mcp_connection_close(c.handle, C.bool(flush))
	if result != C.MCP_OK {
...
func (c *Connection) GetStats() (bytesRead, bytesWritten uint64, err error) {
	...
	result := C.mcp_connection_get_stats(c.handle, &read, &written)
	if result != C.MCP_OK {
...
func (c *Connection) destroy() {
	if c.handle != nil {
		C.mcp_connection_destroy(c.handle)
		c.handle = nil
		if c.callbackID != 0 {
			unregisterCallback(c.callbackID)
		}
...
func goConnectionStateCallback(conn C.mcp_connection_t, oldState, newState C.int, userData unsafe.Pointer) {
	id := uintptr(userData)
	if cb := getCallback(id); cb != nil {
		if c, ok := cb.(*Connection); ok && c.callbacks.OnStateChange != nil {
			c.callbacks.OnStateChange(ConnectionState(oldState), ConnectionState(newState))
		}
	}
}

//export goDataCallback
func goDataCallback(conn C.mcp_connection_t, data *C.uchar, length C.size_t, userData unsafe.Pointer) {
	id := uintptr(userData)
	if cb := getCallback(id); cb != nil {
		if c, ok := cb.(*Connection); ok && c.callbacks.OnData != nil {
...
func goErrorCallback(errorCode C.int, message *C.char, userData unsafe.Pointer) {
	id := uintptr(userData)
	if cb := getCallback(id); cb != nil {
		if c, ok := cb.(*Connection); ok && c.callbacks.OnError != nil {
			err := fmt.Errorf("MCP error %d", errorCode)
			msg := C.GoString(message)
			c.callbacks.OnError(err, msg)
		}
	}
}
...
func (d *Dispatcher) CreateClient(config ClientConfig) (*MCPClient, error) {
	// Create connection
	conn, err := d.CreateConnection(config.Transport)
	if err != nil {
		return nil, err
	}
...
func (c *MCPClient) Connect() error {
	...
	err := c.connection.SetCallbacks(ConnectionCallbacks{
		OnStateChange: func(oldState, newState ConnectionState) {
			// Handle state changes
		},
		OnData: func(data []byte) {
			// Parse and handle MCP messages
