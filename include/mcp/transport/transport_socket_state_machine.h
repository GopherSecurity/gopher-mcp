...
protected:
   ...
   */
  virtual std::unordered_set<TransportSocketState> 
  getValidTransitions(TransportSocketState from) const;
  ...
  std::atomic<TransportSocketState> current_state_{TransportSocketState::Uninitialized};
...
public:
   ...
   */
  virtual std::unique_ptr<TransportSocketStateMachine> 
  createStateMachine(event::Dispatcher& dispatcher,
                    StateMachineConfig config) = 0;
...
public:
   ...
   */
  static bool runCommonScenarios(
