...
struct TransportCapability {
  ...
  TransportCapability(TransportType t, const std::string& n, const std::string& v)
...
class TransportCapabilities {
 public:
   ...
   */
  optional<TransportCapability> negotiate(const TransportPreferences& prefs) const {
  ...
  static TransportCapabilities fromMetadata(const Metadata& meta) {
    ...
    if (transports_it != meta.end() && transports_it->second.isArray()) {
      for (const auto& transport_meta : transports_it->second.asArray()) {
        if (!transport_meta.isObject()) continue;
 ...
 private:
  ...
  static TransportType transportTypeFromString(const std::string& str) {
    if (str == "stdio") return TransportType::STDIO;
    if (str == "http+sse") return TransportType::HTTP_SSE;
    if (str == "websocket") return TransportType::WEBSOCKET;
    if (str == "tcp") return TransportType::TCP;
    if (str == "unix") return TransportType::UNIX;
  ...
  static TransportCapabilitiesPtr createFromConfig(const Metadata& config) {
    ...
    if (transports != config.end() && transports->second.isArray()) {
      for (const auto& transport : transports->second.asArray()) {
        if (!transport.isObject()) continue;
        
        auto type_it = transport.find("type");
        if (type_it == transport.end() || !type_it->second.isString()) continue;
