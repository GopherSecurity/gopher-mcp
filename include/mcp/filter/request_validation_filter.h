...
public:
   ...
   */
  RequestValidationFilter(ValidationCallbacks& callbacks,
...
private:
  bool validateRequest(const jsonrpc::Request& request) {
    ...
    if (config_.validate_protocol_version) {
      if (request.jsonrpc != config_.required_protocol_version) {
        callbacks_.onRequestRejected(request.method, 
            "Invalid protocol version: " + request.jsonrpc);
  ...
  void sendErrorResponse(const RequestId& id, int code, const std::string& message) {
