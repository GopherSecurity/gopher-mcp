...
struct HttpCodecStateMachineConfig {
  ...
  // Callbacks
  std::function<void(const HttpCodecStateTransitionContext&)> state_change_callback;
...
public:
  using StateChangeCallback = std::function<void(const HttpCodecStateTransitionContext&)>;
  using CompletionCallback = std::function<void(bool success)>;
  using ValidationCallback = std::function<bool(HttpCodecState from, HttpCodecState to)>;
  using StateAction = std::function<void(HttpCodecState state, std::function<void()> done)>;
   ...
   */
  HttpCodecStateTransitionResult handleEvent(HttpCodecEvent event,
   ...
   */
  HttpCodecStateTransitionResult transitionTo(HttpCodecState new_state,
  ...
  bool hasError() const {
    return current_state_ == HttpCodecState::Error;
  }
   ...
   */
  bool isTransitionValid(HttpCodecState from, HttpCodecState to,
