...
struct CircuitBreakerConfig {
  ...
  // Timing
  std::chrono::milliseconds timeout = std::chrono::seconds(30);  // Time before trying half-open
  std::chrono::milliseconds window_size = std::chrono::seconds(60);  // Sliding window for metrics
...
public:
   ...
   */
  CircuitBreakerFilter(Callbacks& callbacks,
  ...
  void onResponse(const jsonrpc::Response& response) override {
    ...
    if (it != pending_requests_.end()) {
      ...
      auto latency = std::chrono::duration_cast<std::chrono::milliseconds>(
          std::chrono::steady_clock::now() - req_info.start_time).count();
      ...
      if (is_error && response.error.has_value()) {
        ...
        is_client_error = (code >= -32099 && code <= -32000) ||
                         (code >= 400 && code < 500);
      }
    ...
    switch (state_) {
      ...
      case CircuitState::OPEN:
        ...
        if (std::chrono::duration_cast<std::chrono::milliseconds>(
          ...
          transitionState(CircuitState::HALF_OPEN, "Timeout expired, testing recovery");
          half_open_requests_ = 0;
          half_open_successes_ = 0;
          return true;
        }
