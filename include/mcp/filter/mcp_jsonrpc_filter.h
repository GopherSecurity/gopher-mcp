 ...
 * ```
 * [TCP Socket] → [HTTP Filter] → [SSE Filter] → [MCP JSON-RPC Filter] → [Application]
 *  (Raw I/O)      (HTTP layer)    (SSE layer)     (JSON-RPC layer)       (Business logic)
 * ```
 * 
 * Responsibilities:
...
public:
  ...
  public:
     ...
     * @return Success or error
     */
    virtual VoidResult encodeNotification(const jsonrpc::Notification& notification) = 0;
  ...
  ~McpJsonRpcFilter() override;  // Defined in .cc to avoid incomplete type issues
