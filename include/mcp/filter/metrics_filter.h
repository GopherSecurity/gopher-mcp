...
private:
  ...
  void updateLatencyMetrics(uint64_t latency_ms) {
    ...
    if (latency_ms > metrics_.max_latency_ms) {
      ...
      if (latency_ms > config_.max_latency_threshold_ms) {
        callbacks_.onThresholdExceeded("latency_ms", 
                                      latency_ms,
  ...
  void checkErrorThreshold() {
    ...
    if (age.count() > 0) {
      uint64_t error_rate = (metrics_.errors_received + metrics_.protocol_errors) 
                           / age.count();
      if (error_rate > config_.error_rate_threshold) {
        callbacks_.onThresholdExceeded("error_rate", 
                                      error_rate,
