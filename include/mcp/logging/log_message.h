...
public:
  ...
  void merge(const LogContext& other) {
    if (!other.trace_id.empty()) trace_id = other.trace_id;
    if (!other.span_id.empty()) span_id = other.span_id;
    if (!other.connection_id.empty()) connection_id = other.connection_id;
    if (!other.stream_id.empty()) stream_id = other.stream_id;
    if (!other.request_id.empty()) request_id = other.request_id;
    if (!other.session_id.empty()) session_id = other.session_id;
    if (!other.tool_name.empty()) tool_name = other.tool_name;
    if (!other.resource_uri.empty()) resource_uri = other.resource_uri;
  ...
  LogMessage toLogMessage(LogLevel level, const std::string& msg) const {
    ...
    log_msg.accumulated_latency = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(accumulated_latency).count();
    log_msg.metadata = metadata;
    
    return log_msg;
  }
  
  LogContext() : thread_id(std::this_thread::get_id()),
