...
struct mcp_deleter<std::remove_pointer_t<mcp_notification_t>> {
    void operator()(std::remove_pointer_t<mcp_notification_t>* notif) const noexcept {
        if (notif) {
...
public:
    ...
    MemoryPoolGuard(MemoryPoolGuard&& other) noexcept 
        : pool_(other.pool_) {
        other.pool_ = nullptr;
    }
...
public:
    ...
    ~ErrorContextGuard() noexcept {
        mcp_clear_last_error();
    }
    
    const mcp_error_info_t* get() const noexcept {
...
inline auto validate_and_guard(T* resource, mcp_type_id_t expected_type) {
    if (resource && mcp_validate_handle(reinterpret_cast<mcp_handle_t>(resource), expected_type)) {
