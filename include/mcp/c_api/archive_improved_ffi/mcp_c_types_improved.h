...
MCP_API mcp_json_t mcp_json_array_get(mcp_json_t json,
                                      size_t index) MCP_NOEXCEPT;
MCP_API mcp_result_t mcp_json_array_push(mcp_json_t json,
                                         mcp_json_t value) MCP_NOEXCEPT;

/* JSON object operations */
MCP_API size_t mcp_json_object_size(mcp_json_t json) MCP_NOEXCEPT;
MCP_API mcp_json_t mcp_json_object_get(mcp_json_t json,
                                       const char* key) MCP_NOEXCEPT;
MCP_API mcp_result_t mcp_json_object_set(mcp_json_t json,
                                         const char* key,
                                         mcp_json_t value) MCP_NOEXCEPT;
MCP_API mcp_bool_t mcp_json_object_has(mcp_json_t json,
                                       const char* key) MCP_NOEXCEPT;
...
typedef struct mcp_result_impl*
    mcp_result_obj_t; /* Renamed to avoid conflict */
...
MCP_API mcp_result_t mcp_request_set_id(mcp_request_t request,
                                        mcp_request_id_t id) MCP_NOEXCEPT;
MCP_API mcp_result_t mcp_request_set_params(mcp_request_t request,
                                            mcp_json_t params) MCP_NOEXCEPT;
...
MCP_API mcp_response_t mcp_response_create_success(
    mcp_request_id_t id, mcp_json_t result) MCP_NOEXCEPT;
MCP_API mcp_response_t
mcp_response_create_error(mcp_request_id_t id, mcp_error_t error) MCP_NOEXCEPT;
MCP_API void mcp_response_free(mcp_response_t response) MCP_NOEXCEPT;
MCP_API mcp_request_id_t mcp_response_get_id(mcp_response_t response)
    MCP_NOEXCEPT;
MCP_API mcp_bool_t mcp_response_is_success(mcp_response_t response)
    MCP_NOEXCEPT;
MCP_API mcp_bool_t mcp_response_is_error(mcp_response_t response) MCP_NOEXCEPT;
MCP_API mcp_json_t mcp_response_get_result(mcp_response_t response)
    MCP_NOEXCEPT;
MCP_API mcp_error_t mcp_response_get_error(mcp_response_t response)
    MCP_NOEXCEPT;
...
MCP_API mcp_notification_t mcp_notification_create(const char* method)
    MCP_NOEXCEPT;
MCP_API void mcp_notification_free(mcp_notification_t notification)
    MCP_NOEXCEPT;
MCP_API mcp_result_t mcp_notification_set_params(
    mcp_notification_t notification, mcp_json_t params) MCP_NOEXCEPT;
MCP_API const char* mcp_notification_get_method(mcp_notification_t notification)
    MCP_NOEXCEPT;
MCP_API mcp_json_t mcp_notification_get_params(mcp_notification_t notification)
    MCP_NOEXCEPT;
MCP_API mcp_bool_t mcp_notification_is_valid(mcp_notification_t notification)
    MCP_NOEXCEPT;
...
MCP_API mcp_error_t mcp_error_create_with_data(int32_t code,
                                               const char* message,
                                               mcp_json_t data) MCP_NOEXCEPT;
 ...
 */
MCP_API mcp_string_owned_t* mcp_string_move(
    mcp_string_owned_t* from, mcp_string_owned_t* to) MCP_NOEXCEPT;
...
MCP_API void mcp_batch_execute(const mcp_batch_operation_t* operations,
                               size_t count) MCP_NOEXCEPT;

/**
 * Validation helpers for defensive programming
 */
MCP_API mcp_bool_t mcp_validate_handle(
    mcp_handle_t handle, mcp_type_id_t expected_type) MCP_NOEXCEPT;
MCP_API mcp_bool_t mcp_validate_string(const mcp_string_view_t* str)
    MCP_NOEXCEPT;
MCP_API mcp_bool_t mcp_validate_memory_range(const void* ptr,
...
MCP_API mcp_singleton_t
mcp_singleton_get_or_create(const char* name,
                            mcp_singleton_t (*creator)(void),
                            void (*deleter)(mcp_singleton_t)) MCP_NOEXCEPT;
...
MCP_API mcp_scoped_lock_t mcp_scoped_lock_acquire(const char* resource_name)
    MCP_NOEXCEPT;
...
MCP_API mcp_string_owned_t* mcp_string_create_from_pool(
    mcp_mempool_t pool, const char* data, size_t length) MCP_NOEXCEPT;

/**
 * Pool-allocated list for efficient list operations
 */
MCP_API mcp_list_t mcp_list_create_from_pool(mcp_mempool_t pool,
 ...
 */
MCP_API mcp_map_t mcp_map_create_from_pool(mcp_mempool_t pool,
...
MCP_API void mcp_set_lifecycle_callbacks(
    mcp_type_id_t type,
    const mcp_lifecycle_callbacks_t* callbacks) MCP_NOEXCEPT;
...
struct StringDeleter {
  void operator()(mcp_string_owned_t* s) const noexcept { mcp_string_free(s); }
};

struct ListDeleter {
  void operator()(mcp_list_t l) const noexcept { mcp_list_free(l); }
};

struct MapDeleter {
  void operator()(mcp_map_t m) const noexcept { mcp_map_free(m); }
};

struct JsonDeleter {
  void operator()(mcp_json_t j) const noexcept { mcp_json_free(j); }
};

struct RequestDeleter {
  void operator()(mcp_request_t r) const noexcept { mcp_request_free(r); }
};

struct ResponseDeleter {
  void operator()(mcp_response_t r) const noexcept { mcp_response_free(r); }
};

/* Type aliases for RAII */
using UniqueString = std::unique_ptr<mcp_string_owned_t, StringDeleter>;
using UniqueList =
    std::unique_ptr<std::remove_pointer_t<mcp_list_t>, ListDeleter>;
using UniqueMap = std::unique_ptr<std::remove_pointer_t<mcp_map_t>, MapDeleter>;
using UniqueJson =
    std::unique_ptr<std::remove_pointer_t<mcp_json_t>, JsonDeleter>;
using UniqueRequest =
    std::unique_ptr<std::remove_pointer_t<mcp_request_t>, RequestDeleter>;
using UniqueResponse =
    std::unique_ptr<std::remove_pointer_t<mcp_response_t>, ResponseDeleter>;
