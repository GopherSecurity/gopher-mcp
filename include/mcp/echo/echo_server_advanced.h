...
public:
  ...
  optional<jsonrpc::Notification> processNotification(const jsonrpc::Notification& notification);
  
private:
  Metadata createEchoMetadata(const std::string& method, const optional<Metadata>& params);
...
public:
  AdvancedEchoServer(EchoTransportPtr transport, const EchoServerConfig& config = {});
  ~AdvancedEchoServer();
...
private:
  ...
  void handleNotification(const jsonrpc::Notification& notification, ConnectionContext* context);
  void sendResponse(const jsonrpc::Response& response, ConnectionContext* context);
  void sendNotification(const jsonrpc::Notification& notification, ConnectionContext* context);
  ...
  // Custom handlers
  std::unordered_map<std::string, std::function<jsonrpc::Response(const jsonrpc::Request&)>> request_handlers_;
  std::unordered_map<std::string, std::function<void(const jsonrpc::Notification&)>> notification_handlers_;
