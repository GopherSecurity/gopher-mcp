...
protected:
  ...
  virtual void handleNotification(const jsonrpc::Notification& notification) {
    ...
    if (config_.echo_notifications) {
      auto echo = make<jsonrpc::Notification>("echo/" + notification.method)
  ...
  void logInfo(const std::string& message) {
    if (config_.enable_logging) {
      std::cerr << "[INFO] " << config_.server_name << ": " << message << std::endl;
    }
  }
  ...
  void logError(const std::string& message) {
    if (config_.enable_logging) {
      std::cerr << "[ERROR] " << config_.server_name << ": " << message << std::endl;
    }
  }
...
private:
  void setupCallbacks() {
    ...
    transport_->setDataCallback([this](const std::string& data) {
  ...
  void processJsonRpcMessage(const std::string& message) {
    try {
      ...
      try {
        ...
        if (json_val.contains("id")) {
          auto id = json_val["id"];
          auto error_response = make<jsonrpc::Response>(0)  // Will be overridden
  ...
  void sendServerReady() {
    auto notification = make<jsonrpc::Notification>("server.ready")
        .params(make<Metadata>()
            .add("server", config_.server_name)
...
public:
  ...
  std::future<jsonrpc::Response> sendRequest(const std::string& method,
    ...
    auto request = make<jsonrpc::Request>(id, method)
        .params(params)
        .build();
  ...
  void sendNotification(const std::string& method, const Metadata& params = {}) {
    ...
    auto notification = make<jsonrpc::Notification>(method)
        .params(params)
        .build();
...
protected:
  ...
  virtual void handleRequest(const jsonrpc::Request& request) {
    ...
    // Send error response
    auto response = make<jsonrpc::Response>(request.id)
        .error(Error(jsonrpc::METHOD_NOT_FOUND, "Client doesn't handle requests"))
        .build();
  ...
  void logInfo(const std::string& message) {
    if (config_.enable_logging) {
      std::cerr << "[INFO] " << config_.client_name << ": " << message << std::endl;
    }
  }
  ...
  void logError(const std::string& message) {
    if (config_.enable_logging) {
      std::cerr << "[ERROR] " << config_.client_name << ": " << message << std::endl;
    }
  }
  ...
  void handleResponse(const jsonrpc::Response& response) {
    ...
    if (it != pending_requests_.end()) {
      ...
      if (response.error.has_value()) {
        logError("Request #" + std::to_string(id) + " failed: " + response.error->message);
      } else {
        logInfo("Request #" + std::to_string(id) + " completed in " + std::to_string(ms) + "ms");
      }
      ...
      logError("Received response for unknown request ID: " + std::to_string(id));
    }
  }
