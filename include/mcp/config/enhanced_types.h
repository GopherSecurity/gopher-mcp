...
struct NodeConfigEnhanced : public NodeConfig {
    static NodeConfigEnhanced fromJson(const nlohmann::json& j, ParseContext& ctx) {
        ...
        if (j.contains("metadata")) {
            ...
            validateJsonType(j["metadata"], nlohmann::json::value_t::object, "metadata", ctx);
            
            try {
                config.metadata = j["metadata"].get<std::map<std::string, std::string>>();
            } catch (const nlohmann::json::exception& e) {
                throw ctx.createError("Invalid metadata format: " + std::string(e.what()));
            }
        }
        ...
        try {
            ...
            throw ConfigParseError(e.reason(), ctx.getCurrentPath() + "." + e.field());
        }
...
struct CapabilitiesConfigEnhanced : public CapabilitiesConfig {
    static CapabilitiesConfigEnhanced fromJson(const nlohmann::json& j, ParseContext& ctx) {
        CapabilitiesConfigEnhanced config;
        
        // Features array
        if (j.contains("features")) {
            ParseContext::FieldScope scope(ctx, "features");
            validateJsonType(j["features"], nlohmann::json::value_t::array, "features", ctx);
            
            try {
                config.features = j["features"].get<std::vector<std::string>>();
            } catch (const nlohmann::json::exception& e) {
                throw ctx.createError("Invalid features array: " + std::string(e.what()));
            }
        }
        ...
        if (j.contains("max_request_size")) {
            ...
            try {
                config.max_request_size = parseJsonSize<size_t>(j["max_request_size"], "max_request_size");
        ...
        if (j.contains("max_response_size")) {
            ...
            try {
                config.max_response_size = parseJsonSize<size_t>(j["max_response_size"], "max_response_size");
            } catch (const UnitParseError& e) {
                throw ctx.createError("Invalid size format: " + std::string(e.what()));
            }
        }
        ...
        if (j.contains("request_timeout_ms")) {
            ...
            try {
                config.request_timeout_ms = parseJsonDuration<uint32_t>(j["request_timeout_ms"], "request_timeout_ms");
            } catch (const UnitParseError& e) {
                throw ctx.createError("Invalid duration format: " + std::string(e.what()));
            }
        } else if (j.contains("request_timeout")) {
            ParseContext::FieldScope scope(ctx, "request_timeout");
            try {
                config.request_timeout_ms = parseJsonDuration<uint32_t>(j["request_timeout"], "request_timeout");
            } catch (const UnitParseError& e) {
                throw ctx.createError("Invalid duration format: " + std::string(e.what()));
            }
        }
...
struct FilterConfigEnhanced : public FilterConfig {
    static FilterConfigEnhanced fromJson(const nlohmann::json& j, ParseContext& ctx) {
        ...
        if (j.contains("config")) {
            ...
            validateJsonType(j["config"], nlohmann::json::value_t::object, "config", ctx);
            ...
            if (config.type == "buffer" && config.config.contains("max_size")) {
                try {
                    auto size = parseJsonSize<size_t>(config.config["max_size"], "max_size");
                    config.config["max_size"] = size;
                } catch (const UnitParseError& e) {
                    throw ctx.createError("Invalid buffer size: " + std::string(e.what()));
                }
            }
            ...
            if (config.type == "rate_limit") {
                if (config.config.contains("window_duration")) {
                    try {
                        auto duration = parseJsonDuration<uint32_t>(config.config["window_duration"], "window_duration");
                        config.config["window_duration"] = duration;
                    } catch (const UnitParseError& e) {
                        throw ctx.createError("Invalid window duration: " + std::string(e.what()));
                    }
                }
            }
        }
...
struct ServerConfigEnhanced : public ServerConfig {
    static ServerConfigEnhanced fromJson(const nlohmann::json& j, ParseContext& ctx) {
        ...
        if (j.contains("version")) {
            ...
            try {
                ...
                throw ctx.createError("Invalid version format: " + std::string(e.what()));
            } catch (const nlohmann::json::exception& e) {
        ...
        if (j.contains("capabilities")) {
            config.capabilities = ConfigParser<CapabilitiesConfigEnhanced>::parseField(j, "capabilities", ctx);
        }
        ...
        if (j.contains("session_timeout_ms")) {
            ...
            try {
                config.session_timeout_ms = parseJsonDuration<uint32_t>(j["session_timeout_ms"], "session_timeout_ms");
            } catch (const UnitParseError& e) {
            ...
            try {
                config.session_timeout_ms = parseJsonDuration<uint32_t>(j["session_timeout"], "session_timeout");
            } catch (const UnitParseError& e) {
                throw ctx.createError("Invalid duration: " + std::string(e.what()));
            }
        }
        ...
        if (j.contains("filter_chains")) {
            ...
            validateJsonType(j["filter_chains"], nlohmann::json::value_t::array, "filter_chains", ctx);
                ...
                try {
                    ...
                    if (chains[i].contains("filters")) {
                        ...
                        for (size_t j = 0; j < filters.size(); ++j) {
                            ParseContext::FieldScope filter_scope(ctx, "filters[" + std::to_string(j) + "]");
                            chain.filters.push_back(FilterConfigEnhanced::fromJson(filters[j], ctx));
                        }
                    }
                    
                    config.filter_chains.push_back(chain);
                } catch (const ConfigValidationError& e) {
                    throw ctx.createError("Invalid filter chain: " + std::string(e.what()));
                }
            }
        }
        ...
        if (j.contains("transports")) {
            ...
            validateJsonType(j["transports"], nlohmann::json::value_t::array, "transports", ctx);
