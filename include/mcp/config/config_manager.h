...
public:
    explicit FileConfigSource(const std::string& filepath, 
                              Priority priority = Priority::FILE);
...
private:
    ...
    int priority_;
    mutable std::chrono::system_clock::time_point last_check_;
...
public:
     ...
     */
    bool initialize(const std::vector<std::shared_ptr<ConfigSource>>& sources = {},
