...
public:
    ...
    class FieldScope {
    public:
        FieldScope(ParseContext& ctx, const std::string& field)
            : ctx_(ctx) {
 ...
 */
template<typename T>
T getJsonField(const nlohmann::json& j, 
               const std::string& field,
               ParseContext& ctx) {
    ...
    try {
        ...
        throw ctx.createError("Failed to parse field '" + field + 
 ...
 */
template<typename T>
bool getOptionalJsonField(const nlohmann::json& j,
                          const std::string& field,
                          T& value,
                          ParseContext& ctx) {
    if (!j.contains(field)) {
...
ConfigType parseJsonWithContext(const nlohmann::json& j,
...
class ConfigParser {
public:
    static ConfigType parse(const nlohmann::json& j, ParseContext& ctx) {
        try {
            return ConfigType::fromJson(j);
        } catch (const ConfigValidationError& e) {
    ...
    static ConfigType parseField(const nlohmann::json& parent,
                                  const std::string& field,
                                  ParseContext& ctx) {
        ParseContext::FieldScope scope(ctx, field);
    ...
    static bool parseOptionalField(const nlohmann::json& parent,
                                    const std::string& field,
                                    ConfigType& result,
                                    ParseContext& ctx) {
        if (!parent.contains(field)) {
...
ConfigType loadConfigWithDiagnostics(const std::string& filename) {
    ...
    try {
        ...
        throw ctx.createError("Failed to parse configuration: " + std::string(e.what()));
    }
}
 ...
 */
inline std::string getJsonExcerpt(const nlohmann::json& j, size_t max_length = 100) {
 ...
 */
inline void validateJsonType(const nlohmann::json& j,
                              nlohmann::json::value_t expected_type,
                              const std::string& field,
                              ParseContext& ctx) {
    if (j.type() != expected_type) {
        std::string actual = j.type_name();
        std::string expected;
        ...
        switch (expected_type) {
            ...
            case nlohmann::json::value_t::object:
                expected = "object"; break;
            case nlohmann::json::value_t::array:
                expected = "array"; break;
            case nlohmann::json::value_t::string:
                expected = "string"; break;
            case nlohmann::json::value_t::boolean:
                expected = "boolean"; break;
            case nlohmann::json::value_t::number_integer:
            case nlohmann::json::value_t::number_unsigned:
            case nlohmann::json::value_t::number_float:
                expected = "number"; break;
            default:
                expected = "unknown"; break;
        }
        
        throw ctx.createError("Expected " + expected + " for field '" + 
                              field + "', got " + actual);
    }
}
