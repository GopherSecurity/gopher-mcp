...
struct McpServerConfig : public application::ApplicationBase::Config {
  ...
  std::vector<TransportType> supported_transports = {
    TransportType::Stdio,
...
public:
  ...
  const optional<Implementation>& getClientInfo() const {
    return client_info_;
  }
...
public:
  ...
  ListResourcesResult listResources(const optional<Cursor>& cursor = nullopt) {
    ...
    for (auto it = resources_.begin(); it != resources_.end() && count < page_size; ++it) {
      if (start > 0) {
        start--;
        continue;
      }
  ...
  ReadResourceResult readResource(const std::string& uri) {
    ...
    if (it != resources_.end()) {
      ...
      content.text = "Resource content for: " + uri;  // Actual implementation would read real content
...
private:
  ...
  std::map<std::string, std::set<std::string>> subscriptions_;  // uri -> session_ids
  std::map<std::string, std::set<std::string>> pending_updates_;  // session_id -> uris
...
class ToolRegistry {
public:
  using ToolHandler = std::function<CallToolResult(const std::string& name,
    ...
    if (it != tool_handlers_.end()) {
      try {
        ...
        error_result.content.push_back(
            ExtendedContentBlock(TextContent("Tool execution failed: " + std::string(e.what()))));
        return error_result;
      }
    }
...
private:
  ...
  std::map<std::string, std::function<jsonrpc::Response(const jsonrpc::Request&,
                                                        SessionContext&)>> request_handlers_;
  std::map<std::string, std::function<void(const jsonrpc::Notification&,
                                           SessionContext&)>> notification_handlers_;
