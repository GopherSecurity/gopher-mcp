...
public:
  ...
  // IoHandle interface
  IoCallResult readv(size_t max_length, RawSlice* slices,
                     size_t num_slices) override;
  IoCallResult read(Buffer& buffer,
                    optional<size_t> max_length = nullopt) override;
  IoCallResult writev(const ConstRawSlice* slices,
                      size_t num_slices) override;
  IoCallResult write(Buffer& buffer) override;
  
  IoCallResult sendmsg(const ConstRawSlice* slices, size_t num_slices,
                       int flags, const Address::Ip* self_ip,
                       const Address::Instance& peer_address) override;
  IoCallResult recvmsg(RawSlice* slices, size_t num_slices,
                       uint32_t self_port,
