 ...
 */
class FilterCallbacks : public ReadFilterCallbacks, public WriteFilterCallbacks {
public:
...
public:
  ...
  void setUpstreamHost(const std::string& host) override { upstream_host_ = host; }
...
public:
   ...
   */
  virtual bool createNetworkFilterChain(FilterManager& filter_manager,
                                        const std::vector<FilterFactoryCb>& factories) const = 0;

  /**
   * Create listener filter chain
   */
  virtual bool createListenerFilterChain(FilterManager& filter_manager) const = 0;
};
