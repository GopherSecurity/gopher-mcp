 ...
 *
 * This state machine manages the lifecycle of filter chains in network connections,
 * coordinating filter initialization, data flow, and termination. It follows
 * industrial proxy design patterns with:
...
private:
  ...
  bool isValidTransition(FilterChainState from, 
...
public:
  ...
  FilterChainBuilder& withErrorCallback(std::function<void(const std::string&)> cb);
